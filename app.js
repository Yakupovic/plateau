...
function MiniLine({
  ...
  if (refY != null) {
    min = Math.min(min, refY);
    max = Math.max(max, refY);
  }
  if (min === max) {
    min -= 1;
    max += 1;
  ...
  return /*#__PURE__*/React.createElement("svg", {
    ...
    fill: C.dim,
    textAnchor: "end"
  }, fmtV(g)))), refY != null && /*#__PURE__*/React.createElement("line", {
    x1: padL,
    x2: W - padR,
    y1: y(refY),
    y2: y(refY),
    ...
    stroke: "#fff",
    strokeWidth: "1.5"
  }), data.length <= 8 && /*#__PURE__*/React.createElement("text", {
    x: x(i),
    y: y(d[dataKey]) - 8,
    fontSize: "9",
    fontWeight: "700",
    fill: textColor,
    textAnchor: "middle"
  }, fmtV(d[dataKey])), i % showXEvery === 0 && /*#__PURE__*/React.createElement("text", {
    x: x(i),
    y: H - 8,
    fontSize: "9",
    fill: C.dim,
...
const exoLine = e => e.type === "cardio" ? `${e.nom} : ${e.dureeCardio} min` + `${e.distance ? `, ${fmtKg(e.distance)} km` : ""}` + `${e.niveau ? `, niveau ${e.niveau}` : ""}` + `${e.ressenti ? ` (${e.ressenti === "marge" ? "facile" : "ça piquait"})` : ""}` + `${e.note ? ` — ${e.note}` : ""}` : `${e.nom} : ${e.series}×${e.reps} @ ${fmtKg(e.poids)} kg${e.parBras ? "/bras" : ""}` + `${e.reposSec ? `, repos ${fmtRepos(e.reposSec)}` : ""}` + `${e.rpe ? `, RPE ${e.rpe}/10` : ""}` + `${e.dureeMin ? `, ~${e.dureeMin} min sur l'exo` : ""}` + `${e.ressenti ? ` (${e.ressenti === "marge" ? "de la marge" : "ça tirait"})` : ""}` + `${e.pr ? " [RECORD PERSO]" : ""}` + `${e.note ? ` — ${e.note}` : ""}`;
const histText = (seances, n = 5) => seances.slice(-n).map(s => `${s.date} — Séance ${s.nom}${s.duree ? `, ${fmtDuree(s.duree)}` : ""}${s.gourdes != null ? `, ${s.gourdes} gourde(s)` : ""}\n` + (s.note ? `  Note de l’utilisateur : ${s.note}\n` : "") + s.exos.map(e => "  - " + exoLine(e)).join("\n")).join("\n\n");
...
const streakSemaines = d => {
  ...
  while (true) {
    ...
    if ((parSemaine[isoOf(w)] || 0) >= 2) streak++;else break;
  }
  return streak;
};
...
const poidsText = d => {
  ...
  return `Poids de corps : ${fmtKg(last.kg)} kg le ${last.date}` + (prev ? ` (précédent : ${fmtKg(prev.kg)} kg le ${prev.date})` : "") + ` — objectif ${OBJECTIF_POIDS} kg.`;
};
const mensuText = d => {
  const arr = d.mensu || [];
...
const echauffementPaliers = cible => {
  ...
  return [{
    pct: 40,
    poids: Math.max(2.5, arrondi25(cible * 0.4)),
    reps: 12
  }, {
    pct: 60,
    poids: Math.max(2.5, arrondi25(cible * 0.6)),
    reps: 8
  }, {
    pct: 80,
    poids: Math.max(2.5, arrondi25(cible * 0.8)),
    reps: 4
  }];
};
...
const best1RM = (d, nom) => {
  ...
  let m = null;
  list.forEach(e => {
    const sets = e.sets && e.sets.length ? e.sets : [{
      poids: e.poids,
      reps: e.reps
    }];
    sets.forEach(s => {
      const v = epley(s.poids, s.reps || 1);
      if (m == null || v > m) m = v;
    });
  });
  return m;
...
const cycleDe = d => {
  ...
  return {
    debut,
    sem,
    cycle: Math.floor((semTotal - 1) / CYCLE_SEMAINES) + 1,
    deload: sem === CYCLE_SEMAINES
  };
};
const cycleText = d => {
  const c = cycleDe(d);
  return c.deload ? `Cycle ${c.cycle}, semaine ${c.sem}/${CYCLE_SEMAINES} : SEMAINE ALLÉGÉE (charges -10 %, 3 séries au lieu de 4) pour récupérer avant de repartir.` : `Cycle ${c.cycle}, semaine ${c.sem}/${CYCLE_SEMAINES} : phase de montée en charge.`;
};
...
const volumeFin = (d, jours = 14, extra = []) => {
  const dep = new Date();
  dep.setDate(dep.getDate() - (jours - 1));
  const min = isoOf(dep);
  const v = {};
  const add = exos => exos.forEach(e => {
    if (e.type === "cardio") return;
...
const seanceConseillee = d => {
  ...
  if (repos === 0) return {
    type: "repos",
    titre: "Repos",
    raison: "Tu as déjà chargé aujourd'hui — le muscle pousse pendant la récup."
  };
  ...
  const scores = Object.entries(GROUPES_SEANCE).map(([nom, groupes]) => {
    ...
    return {
      ...
      groupes,
      parGroupe,
      penalite: memeQueDernier ? 1000 : 0
    };
  }).sort((a, b) => a.parGroupe + a.penalite - (b.parGroupe + b.penalite));
  const best = scores[0];
  const vide = best.groupes.filter(g => !(v30[g] || 0));
  const raison = vide.length ? `${vide.join(" et ")} : rien depuis 30 jours.` : `${best.groupes[0]} est ton groupe le moins servi en ce moment.`;
...
const volumeSemaines = (d, n = 8) => {
  ...
  return Object.keys(parSem).sort().slice(-n).map(k => ({
    date: fmtShort(k),
    kg: Math.round(parSem[k])
  }));
};
...
const niveauGlobal = xp => {
  ...
  return {
    niveau,
    titre: TITRES_NIVEAU[Math.min(TITRES_NIVEAU.length - 1, niveau - 1)],
    xp,
    xpSuivant,
    pct: Math.max(0, Math.min(100, pct))
  };
...
const rpeSemaines = (d, n = 8) => {
  ...
  return Object.keys(parSem).sort().slice(-n).map(k => ({
    date: fmtShort(k),
    rpe: Math.round(parSem[k].total / parSem[k].n * 10) / 10
  }));
};
...
const buildContext = d => `Historique des séances :\n${histText(d.seances)}\n\n` + `Récup des 7 derniers jours :\n${recupText(d)}\n\n` + `Séries des 7 derniers jours par groupe musculaire : ${musclesText(d)}\n` + `Streak : ${streakSemaines(d)} semaine(s) consécutive(s) avec au moins 2 séances.\n` + (d.prochaine ? `Prochaine séance planifiée : ${d.prochaine.nom} le ${d.prochaine.date}.\n` : "") + poidsText(d) + (mensuText(d) ? `\n${mensuText(d)}` : "") + (objectifsText(d) ? `\n${objectifsText(d)}` : "") + (stagText(d) ? `\n${stagText(d)}` : "") + `\n${cycleText(d)}` + (equilibreText(d) ? `\n${equilibreText(d)}` : "") + (ciblesText(d) ? `\n${ciblesText(d)}` : "");
const COACH_BASE = passages => `Tu es le coach de musculation personnel de l’utilisateur, qui s'entraîne en salle sur machines guidées. ` + `Tu tutoies, ton direct, naturel et chaleureux, en français. Réponds UNIQUEMENT en phrases simples : pas de listes, pas de markdown, pas d'émojis. ` + `Règles de progression : ressenti "de la marge" = proposer +2 à 2,5 kg la prochaine fois ; "ça tirait" = garder la même charge. ` + `Il n'a encore jamais travaillé les jambes (première séance Jambes prévue le 22/07/2026) : encourage-le quand c'est pertinent, sans le harceler. ` + `La douche rapide juste après la séance compte beaucoup pour lui (santé de la peau) : glisse un rappel en fin de séance si elle n'est pas notée comme faite. ` + `Tiens compte de son sommeil, de son alimentation et de l'espacement de ses séances quand ces infos sont fournies : moins de 6 h de sommeil ou une alimentation légère un jour de séance méritent un conseil. ` + `Objectif de poids de corps : ${OBJECTIF_POIDS} kg (prise de masse, entraînement 100 % naturel). Quand un record perso est signalé, félicite sobrement. ` + `Date du jour : ${new Date().toLocaleDateString("fr-FR")}. ` + (passages != null ? `Assiduité : ${passages} passages en salle, prochain palier à 25 (puis 50) — motive-le dessus quand c'est pertinent, sans en faire trop.` : "");
const getApiKey = () => {
  try {
    return localStorage.getItem("pl:apikey") || "";
  } catch {
    return "";
  }
};
...
const photoCache = {};
const compressImage = (file, maxSide = 800, quality = 0.6) => new Promise((resolve, reject) => {
...
function HiitTimer({
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      setEtat(e => {
        ...
        if (e.phase === "work" && preset.rest > 0) return {
          ...e,
          phase: "rest",
          reste: preset.rest
        };
        if (e.round >= preset.rounds) return {
          ...e,
          phase: "fini",
          reste: 0
        };
  ...
  useEffect(() => {
    if (phase === "fini") setEnCours(false);
  }, [phase]);
  const reinit = p => {
    setPreset(p);
    setEnCours(false);
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, HIIT_PRESETS.map(p => /*#__PURE__*/React.createElement(Chip, {
    key: p.nom,
    active: preset.nom === p.nom,
    onClick: () => reinit(p)
  ...
  }, phase === "fini" ? "Terminé" : phase === "work" ? "Effort" : "Repos", phase !== "fini" && ` · round ${round}/${preset.rounds}`), /*#__PURE__*/React.createElement("div", {
    className: "leading-none my-3",
    style: {
      ...DISPLAY,
      ...NUMS,
      fontSize: 84,
      color: phase === "rest" ? C.dim : C.text
    }
  }, phase === "fini" ? "✓" : reste), /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => {
      if (phase === "fini") reinit(preset);else setEnCours(!enCours);
    },
  ...
  }, phase === "fini" ? "Recommencer" : enCours ? "Pause" : "Démarrer")), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-4",
    style: {
      color: C.dim
    }
  }, preset.work, " s d'effort", preset.rest > 0 ? ` · ${preset.rest} s de repos` : "", " \xB7 ", preset.rounds, " rounds"));
}
...
function SchemaMouvement({
  ...
  let content;
  if (k === "presse") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "36",
    y1: "50",
    x2: "80",
    y2: "94"
  }, M)), /*#__PURE__*/React.createElement("line", _extends({
    x1: "80",
    y1: "94",
    x2: "108",
    y2: "105"
  }, M)), /*#__PURE__*/React.createElement("circle", {
    cx: "45",
    cy: "45",
    r: "9",
    fill: SCH.body
    ...
    x2: "88",
    y2: "90"
  }, B)), /*#__PURE__*/React.createElement("line", _extends({
    x1: "88",
    y1: "90",
    x2: "116",
    y2: "66"
    ...
    x2: "158",
    y2: "44"
  }, G)), /*#__PURE__*/React.createElement("line", _extends({
    x1: "148",
    y1: "58",
    x2: "174",
    y2: "34"
    ...
    x2: 166,
    y2: 74
  }));else if (k === "extension-jambes") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "52",
    y1: "40",
    x2: "52",
    y2: "88"
    ...
    x2: "138",
    y2: "83"
  }, G)), /*#__PURE__*/React.createElement("circle", {
    cx: "140",
    cy: "83",
    r: "7",
    fill: "none",
    ...
    x2: 136,
    y2: 98
  }));else if (k === "curl-jambes") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "52",
    y1: "40",
    x2: "52",
    y2: "88"
    ...
    x2: 112,
    y2: 118
  }));else if (k === "mollets") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "68",
    y1: "126",
    x2: "132",
    y2: "126"
    ...
    x2: "110",
    y2: "122"
  }, G)), /*#__PURE__*/React.createElement(Arr, {
    x1: 148,
    y1: 112,
    x2: 148,
    y2: 72
  }));else if (k === "squat") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "60",
    y1: "130",
    x2: "160",
    y2: "130"
    ...
    x2: 108,
    y2: 88
  }));else if (k === "abducteurs") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "70",
    y1: "112",
    x2: "130",
    y2: "112"
    ...
    x2: 142,
    y2: 116
  }));else if (k === "ouverture") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
    cx: "100",
    cy: "32",
    r: "10",
    fill: SCH.body
    ...
    x2: 146,
    y2: 48
  }));else if (k === "ecartes") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
    cx: "100",
    cy: "32",
    r: "10",
    fill: SCH.body
    ...
    x2: 122,
    y2: 34
  }));else if (k === "pulldown") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "58",
    y1: "20",
    x2: "142",
    y2: "20"
    ...
    x2: 134,
    y2: 80
  }));else if (k === "row") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
    cx: "70",
    cy: "50",
    r: "9",
    fill: SCH.body
    ...
    x2: 86,
    y2: 104
  }));else if (k === "press") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "52",
    y1: "38",
    x2: "52",
    y2: "96"
    ...
    x2: 130,
    y2: 80
  }));else if (k === "triceps") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", {
    x1: "58",
    y1: "16",
    x2: "58",
    y2: "44",
    ...
    x2: 110,
    y2: 102
  }));else if (k === "curl") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
    cx: "78",
    cy: "30",
    r: "9",
    fill: SCH.body
    ...
    x2: 132,
    y2: 62
  }));else if (k === "elevation") content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("circle", {
    cx: "100",
    cy: "28",
    r: "10",
    fill: SCH.body
    ...
    x2: 146,
    y2: 62
  }));else content = /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("line", _extends({
    x1: "72",
    y1: "70",
    x2: "128",
    y2: "70"
...
const calculPlaques = (cible, barre) => {
  ...
  return {
    parCote: combo,
    reste: Math.round(restant * 100) / 100,
    atteint: Math.round((barre + total * 2) * 100) / 100
  };
};
...
const PROGRAMMES = {
  ...
  pull: {
    ...
    exos: [{
      ...
      nom: "Tirage Horizontal poulie basse",
      poids: 20,
      series: 4,
      reps: 10,
      reposSec: 90
  ...
  jambes: {
    label: "Jambes (quadriceps/ischios)",
    exos: [{
...
const CATALOGUE = [
// Jambes
"Presse à cuisses", "Hack squat", "Squat barre", "Squat guidé (Smith)", "Fentes haltères", "Leg extension", "Leg curl assis", "Leg curl allongé", "Abducteurs machine", "Adducteurs machine", "Mollets debout", "Mollets assis", "Presse à mollets", "Soulevé de terre roumain",
// Pecs
"Développé couché barre", "Développé couché haltères", "Développé incliné haltères", "Développé incliné barre", "Chest press machine", "Machine convergente", "Pec Deck", "Écartés poulie vis-à-vis", "Dips machine assistée", "Pompes",
// Dos
"Pull Down machine", "Vertical Traction", "Tirage Poitrine machine", "Tirage Horizontal poulie basse", "Low Row", "Rowing haltère", "Rowing barre", "Pull-over poulie", "Tractions assistées", "Shrugs haltères",
// Épaules
"Shoulder Press machine", "Développé militaire barre", "Développé épaules haltères", "Élévations latérales haltères", "Élévations latérales poulie", "Reverse Fly", "Oiseau haltères", "Face pull poulie",
// Biceps
...
const repereFor = (nom, poidsCorps) => {
  ...
  if (!r) return null;
  return {
    paliers: r.m.map(x => Math.round(x * poidsCorps * 2) / 2),
    machine: r.machine
  };
};
...
const Card = ({
  ...
  onClick: onClick,
  className: "pl-card rounded-2xl p-4 " + (className || ""),
  style: style
}, children);
...
function CountUp({
  ...
  const shown = decimals > 0 ? (Math.round(n * 10 ** decimals) / 10 ** decimals).toLocaleString("fr-FR", {
...
const CORPS_CONTOUR = "M80 8 C88.8 8 96 15.2 96 24 C96 32.8 88.8 40 80 40 C71.2 40 64 32.8 64 24 C64 15.2 71.2 8 80 8 Z " + "M69 39 L91 39 L92 50 C104 51 116 56 122 65 C127 73 129 86 128 98 C127 106 122 112 115 113 C112 113 110 111 109 108 " + "C111 96 111 84 108 73 C106 67 102 63 97 61 C99 76 100 92 99 108 C99 122 98 136 96 149 " + "C99 152 100 157 100 162 C102 178 103 194 101 209 C101 217 94 223 87 223 C81 223 77 218 76 211 " + "C75 196 75 181 75 166 C75 181 75 196 74 211 C73 218 69 223 63 223 C56 223 49 217 49 209 " + "C47 194 48 178 50 162 C50 157 51 152 54 149 C52 136 51 122 51 108 C50 92 51 76 53 61 " + "C48 63 44 67 42 73 C39 84 39 96 41 108 C40 111 38 113 35 113 C28 112 23 106 22 98 " + "C21 86 23 73 28 65 C34 56 46 51 58 50 L69 39 Z";
...
function SilhouetteMuscles({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
  ...
  }, dos ? "Vue de dos" : "Vue de face"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setDos(!dos),
  ...
  }, dos ? "Voir la face" : "Voir le dos", " \u21BB")), /*#__PURE__*/React.createElement("svg", {
    viewBox: "0 0 160 236",
    style: {
      width: "100%",
      height: "auto",
    ...
    }, /*#__PURE__*/React.createElement("title", null, m, " \u2014 ", v, " s\xE9rie", v > 1 ? "s" : ""))));
  })), /*#__PURE__*/React.createElement("div", {
  ...
  }), " n\xE9glig\xE9"), /*#__PURE__*/React.createElement("span", {
    className: "flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("i", {
    style: {
      width: 9,
  ...
  }), " travaill\xE9"), /*#__PURE__*/React.createElement("span", {
    className: "flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("i", {
    style: {
      width: 9,
...
function Heatmap({
  ...
  const parJour = useMemo(() => {
    const m = {};
    seances.forEach(s => {
      m[s.date] = (m[s.date] || 0) + tonnageSeance(s);
    });
    return m;
  }, [seances]);
  const cols = [];
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    key: c.iso,
    title: `${fmtShort(c.iso)} — ${c.t ? c.t.toLocaleString("fr-FR") + " kg" : "repos"}`,
    style: {
      width: 11,
      height: 11,
      borderRadius: 3,
      background: teinte(c.t, c.futur),
      border: c.futur ? "none" : `1px solid ${C.hair}`
    }
  }))))));
}

// ————— Radar d'équilibre musculaire —————
function RadarMuscles({
  volumes,
  groupes
}) {
  ...
  }, [0.25, 0.5, 0.75, 1].map((r, k) => /*#__PURE__*/React.createElement("polygon", {
    key: k,
...
const Dot = ({
  ressenti
}) => ressenti ? /*#__PURE__*/React.createElement("span", {
  className: "inline-block w-2 h-2 rounded-full mr-1 align-middle",
  style: {
    background: ressenti === "marge" ? C.green : C.red
  }
}) : null;
const PrBadge = () => /*#__PURE__*/React.createElement("span", {
...
function App() {
      ...
      if (window.storage) {
        ...
        try {
          ...
          if (cur && cur.startedAt) {
            ...
            if (heures > 5) {
              const garder = window.confirm(`Une séance ${cur.nom} est ouverte depuis ${Math.round(heures)} h — tu as sûrement oublié de la terminer.\n\nOK = la reprendre en repartant de maintenant.\nAnnuler = la garder telle quelle.`);
  ...
  const partagerImage = async (dataUrl, titre) => {
    try {
      ...
      const file = new File([blob], "plateau.png", {
        type: "image/png"
      });
  ...
  const scheduleAutoCopie = () => {
    ...
    clipTimer.current = setTimeout(() => {
      ...
      const payload = currentRef.current ? {
        ...dataRef.current,
        enCours: currentRef.current
      } : dataRef.current;
      navigator.clipboard.writeText(JSON.stringify(payload, null, 2)).then(() => setClipState("sync")).catch(() => setClipState("idle"));
    }, 400);
  };
  const saveData = async next => {
    ...
    dataTimer.current = setTimeout(() => {
      ...
      window.storage.set("plateau-data", JSON.stringify(dataRef.current)).then(() => {
        setStorageOk(true);
        flashSaved();
  ...
  const saveCurrent = async cur => {
    ...
    curTimer.current = setTimeout(() => {
      curTimer.current = null;
      window.storage.set("plateau-current", JSON.stringify(currentRef.current)).then(() => {
        setStorageOk(true);
        flashSaved();
      }).catch(() => setStorageOk(false));
    }, 600);
  ...
  const suggestionFor = nom => {
    ...
    if (cycleDe(data).deload) {
      return {
        last,
        cible: Math.round(last.poids * 0.9 * 2) / 2,
        monte: false,
        deload: true
      };
    }
    ...
    try {
      ...
      if (!(err && err.noapi)) setCurrent(prev => {
        ...
        return {
          ...
          exos: prev.exos.map(e => e.id === exo.id ? {
            ...e,
            coach: "Coach injoignable pour le moment — série bien enregistrée."
          } : e)
        };
      });
    } finally {
  ...
  const validerExo = async () => {
    ...
    if (isCardio && !editExoId) {
      ...
      const exo = {
        ...
        pr: false,
        at,
        dureeMin: Math.max(1, Math.round((at - prevAt) / 60000))
      };
      const cur = {
        ...current,
        exos: [...current.exos, exo]
      };
  ...
  const ajouterSeriePrevue = async () => {
    ...
    await saveCurrent({
      ...
      exoEnCours: {
        ...ec,
        prevu: (ec.prevu || ec.sets.length) + 1
      }
    });
  };
  ...
  const finirExoAvec = async (ecArg, curArg) => {
    ...
    const exo = {
      ...
      poids: poidsMax,
      parBras: ec.parBras,
      series: ec.sets.length,
      reps: ec.sets[0].reps,
      reposSec: ec.reposSec,
      ressenti: ec.ressenti,
      coach: null,
      photoId: ec.photoId,
      pr,
      sets: ec.sets,
  ...
  const supprimerExoCourant = async id => {
    ...
    await saveCurrent({
      ...current,
      exos: current.exos.filter(x => x.id !== id)
    });
  };
  const terminer = async () => {
    ...
    let next = {
      ...data,
      seances: [...data.seances, seance]
    };
    if (next.prochaine && next.prochaine.date <= todayISO()) next = {
      ...next,
      prochaine: null
    };
    next = {
  ...
  const supprimerSeance = async id => {
    ...
    await saveData({
      ...data,
      seances: data.seances.filter(s2 => s2.id !== id)
    });
  };
  ...
  const saveEditHist = async () => {
    ...
    const maj = ehCardio ? e => ({
      ...e,
      dureeCardio: ehDuree,
      distance: String(ehDistance).trim() ? parseFloat(String(ehDistance).replace(",", ".")) : null
    }) : e => ({
      ...e,
      poids: parseFloat(String(ehPoids).replace(",", ".")),
      series: ehSeries,
      reps: ehReps,
      sets: null,
      note: e.sets ? null : e.note
    ...
    const next = {
      ...data,
      seances: data.seances.map(s => s.id !== sid ? s : {
        ...s,
        exos: s.exos.map(e => e.id === eid ? maj(e) : e)
      })
    };
    await saveData(next);
    setEhKey(null);
  ...
  const supprimerExoHist = async (sid, eid) => {
    ...
    await saveData({
      ...data,
      seances: data.seances.map(s2 => s2.id !== sid ? s2 : {
        ...s2,
        exos: s2.exos.filter(x => x.id !== eid)
      })
    });
    if (ehKey === sid + "|" + eid) setEhKey(null);
  };
  const renommerExo = async (ancien, nouveau) => {
    const a = ancien.toLowerCase(),
      nv = nouveau.trim();
    if (!nv) return;
    const maj = e => e.nom.toLowerCase() === a ? {
      ...e,
      nom: nv
    } : e;
    const next = {
      ...
      objectifs: (data.objectifs || []).map(o => o.nom.toLowerCase() === a ? {
        ...o,
        nom: nv
      } : o)
    };
    ...
    if (current) await saveCurrent({
      ...current,
      exos: current.exos.map(maj)
    });
  };
  const refaireSeance = s => {
    if (current) {
      window.alert("Une séance est déjà en cours — termine-la d'abord.");
      return;
    }
    setExpandedId(null);
    demarrer(s.nom);
  ...
  const saveNoteHist = async sid => {
    ...
    await saveData({
      ...data,
      seances: data.seances.map(s => s.id === sid ? {
        ...s,
        note: v || null
      } : s)
    });
  ...
  const importerSeanceTexte = async () => {
    ...
    try {
      const rep = await askCoach(`Convertis ce récap de séance de musculation en JSON. Réponds UNIQUEMENT avec un objet JSON valide, sans aucun texte autour et sans balises markdown, au format exact : ` + `{"date":"YYYY-MM-DD ou null","nom":"Push","duree":90,"gourdes":2,"exos":[{"nom":"Pec Deck","poids":10,"parBras":false,"series":4,"reps":10,"reposSec":90,"ressenti":null,"note":null}]}. ` + `Règles : "date" null si absente du texte ; "duree" en minutes, null si absente ; "gourdes" null si absent ; "parBras" true si la charge est indiquée par bras ou par haltère ; ` + `"ressenti" vaut "marge" (facile, de la marge), "tirait" (difficile) ou null ; "note" pour tout détail utile (dégressif, angles…), sinon null.\n\nRécap :\n${importSeanceText.trim()}`);
    ...
    data.seances.forEach(s => {
      s.exos.forEach(e => {
        if (e.nom.toLowerCase() === nom.toLowerCase() && e.type !== "cardio") {
          if (!precedent || s.date >= precedent.date) precedent = {
            date: s.date,
            poids: e.poids
          };
        }
  ...
  const mursRecords = useMemo(() => {
    ...
    data.seances.forEach(s => s.exos.forEach(e => {
      if (e.type === "cardio") return;
      const k = e.nom.toLowerCase();
      if (!best[k] || e.poids > best[k].poids) best[k] = {
        nom: e.nom,
        poids: e.poids,
        date: s.date,
        parBras: e.parBras
  ...
  const planSeance = useMemo(() => {
    ...
    const rang = nom => {
      const i = ordre.indexOf(nom.toLowerCase());
      return i === -1 ? 999 : i;
    };
  ...
  const demanderPlanIA = async () => {
    ...
    setPlanIABusy(true);
    try {
      const rep = await askCoach(`${COACH_BASE(passages)}\n\n${buildContext(data)}\n\n` + `l’utilisateur démarre sa toute première séance "${current.nom}" en salle. ` + `Propose un plan de 5 à 7 exercices adaptés à cette séance, avec des charges de départ prudentes pour une première fois, en cohérence avec son niveau visible dans l'historique. ` + `Réponds UNIQUEMENT avec un tableau JSON valide, sans aucun texte autour et sans balises markdown, au format exact : ` + `[{"nom":"Presse à cuisses","poids":40,"parBras":false,"series":4,"reps":10,"reposSec":120,"conseil":"dos plaqué au dossier, pieds largeur épaules"}] — "conseil" en 12 mots max sur le placement ou réglage machine.`);
      ...
      await saveCurrent({
        ...
        planIA: plan.map(p => ({
          id: uid(),
          nom: String(p.nom || "Exo"),
          poids: Number(p.poids) || 0,
          parBras: !!p.parBras,
          series: Number(p.series) || 4,
          reps: Number(p.reps) || 10,
          reposSec: p.reposSec ? Number(p.reposSec) : 90,
  ...
  const demanderConseil = async type => {
    ...
    setConseil(null);
    try {
      const question = type === "semaine" ? `Fais le bilan complet de sa semaine (7 derniers jours) : séances, volume par groupe musculaire, récupération (sommeil, alimentation, douches), poids de corps, streak et progression Level Up. 6 à 8 phrases, honnête sur les points faibles, et termine par le plan concret de la semaine à venir.` : `Quelle séance conseilles-tu aujourd'hui, et pourquoi ? Réponds en 2 ou 3 phrases.`;
      const rep = await askCoach(`${COACH_BASE(passages)}\n\n${buildContext(data)}\n\n${question}`);
      setConseil(rep);
    } catch (e) {
      setConseil(e && e.noapi ? MSG_NOAPI : "Coach injoignable pour le moment, réessaie dans un instant.");
    } finally {
      setConseilBusy(false);
    }
  };
  ...
  const demanderStrategiePlateau = async (nomExo, poidsStag) => {
    ...
    setStagAvis(null);
    try {
      const rep = await askCoach(`${COACH_BASE(passages)}\n\n${buildContext(data)}\n\n` + `l’utilisateur est en plateau sur "${nomExo}" : 3 séances à ${fmtKg(poidsStag)} kg et ça tire toujours. ` + `Donne ta stratégie de déblocage concrète en 2 ou 3 phrases (deload -10 % et remontée, tempo lent, ou variante d'exercice — choisis).`);
      setStagAvis(rep);
    } catch (e) {
  ...
  const definirCibleMuscle = m => {
    ...
    if (!n || n <= 0) delete cibles[m];else cibles[m] = n;
    saveData({
      ...data,
  ...
  const definirNoteExo = nomExo => {
    ...
    if (!v.trim()) delete notesExo[key];else notesExo[key] = v.trim();
    saveData({
      ...data,
      notesExo
    });
  ...
  const alerteSurentrainement = useMemo(() => {
    ...
    const nuits = Object.keys(data.jours || {}).filter(d => d >= isoMin).map(d => (data.jours[d] || {}).sommeil).filter(h => h != null);
  ...
  const genererCarte = async s => {
    ...
    ctx.fillText(`${fmtDate(s.date)}${s.duree ? " · " + fmtDuree(s.duree) : ""}${s.gourdes != null ? " · " + s.gourdes + " gourde" + (s.gourdes > 1 ? "s" : "") : ""}`, 70, 320);
    ...
    exos.forEach(e => {
      ...
      ctx.fillStyle = e.pr ? "#8a5b00" : "#726c5e";
  ...
  const ajouterPoids = async () => {
    ...
    const arr = [...(data.poids || []).filter(p => p.date !== todayISO()), {
      date: todayISO(),
      kg
  ...
  const ajouterMensu = async () => {
    ...
    const arr = [...(data.mensu || []).filter(m => !(m.date === todayISO() && m.zone === mensuZone)), {
      date: todayISO(),
      zone: mensuZone,
      cm
    }].sort((a, b) => a.date < b.date ? -1 : 1);
  ...
  const supprimerPoids = async date => {
    ...
    await saveData({
      ...data,
      poids: (data.poids || []).filter(p => p.date !== date)
    });
  };
  ...
  const dernieresMensu = ZONES.map(z => {
    ...
    return {
      zone: z,
      last,
      delta: prev ? Math.round((last.cm - prev.cm) * 10) / 10 : null
    };
  });
  ...
  const enregistrerProgramme = async s => {
    ...
    const exos = (s.exos || []).filter(e => e.type !== "cardio").map(e => ({
      ...
      series: e.series || 4,
      reps: e.reps || 10,
      reposSec: e.reposSec || 90
    }));
    if (!exos.length) {
      window.alert("Aucun exercice de muscu à enregistrer dans un programme.");
      return;
    }
    await saveData({
      ...data,
      programmesPerso: [...(data.programmesPerso || []), {
        id: uid(),
        nom: nom.trim(),
        exos
      }]
  ...
  const supprimerProgrammePerso = async id => {
    ...
    await saveData({
      ...data,
      programmesPerso: (data.programmesPerso || []).filter(p => p.id !== id)
    });
  };
  ...
  const copierContexte = async () => {
    const d = dataRef.current;
    const p = PASSAGES_BASE + d.seances.filter(s => s.date > PASSAGES_BASE_DATE).length + (d.passagesOffset || 0);
    const txt = "Voici mes données PLATEAU (mon app de musculation). Réponds-moi comme mon coach.\n\n" + buildContext(d) + `\nla salle : ${p} passages en salle (palier à 25).`;
    try {
      await navigator.clipboard.writeText(txt);
      setCtxCopie(true);
  ...
  const reprendreSauvegarde = async () => {
    try {
      ...
      obj.seances = obj.seances.map(s => ({
        ...s,
        exos: Array.isArray(s.exos) ? s.exos : []
      }));
  ...
  const exporterChiffre = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `plateau-chiffre-${todayISO()}.json`;
    ...
    try {
      ...
      if (essai && essai.plateauChiffre) {
        ...
        try {
          ...
          window.alert("Mot de passe incorrect ou fichier corrompu.");
          return;
        }
      }
  ...
  const chartData = useMemo(() => {
    if (!selExo) return [];
    return data.seances.filter(s => s.exos.some(e => e.nom.toLowerCase() === selExo.toLowerCase())).map(s => ({
  ...
  const definirObjectif = async () => {
    ...
    const arr = [...(data.objectifs || []).filter(o => o.nom.toLowerCase() !== selExo.toLowerCase()), {
      nom: selExo,
      cible
    }];
    await saveData({
  ...
  const retirerObjectif = async () => {
    ...
    await saveData({
      ...data,
      objectifs: (data.objectifs || []).filter(o => o.nom.toLowerCase() !== selExo.toLowerCase())
    });
    setObjInput("");
    setObjEdit(false);
  };
  const chartPoids = useMemo(() => (data.poids || []).map(p => ({
    date: fmtShort(p.date),
    kg: p.kg
  })), [data]);
  const chartMensu = useMemo(() => (data.mensu || []).filter(m => m.zone === progMensuZone).map(m => ({
  ...
  // ————— Rendu —————
  return /*#__PURE__*/React.createElement("div", {
    className: "pl-root min-h-screen w-full" + (data.modeSoleil ? " pl-soleil" : ""),
    style: {
      color: C.text
    }
    ...
    className: "pl-header flex items-end justify-between mb-3"
  ...
  }, "PLATEAU", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.yellowDim
    }
  }, ".")), /*#__PURE__*/React.createElement("div", {
  ...
  }, salutation()), " · ", new Date().toLocaleDateString("fr-FR", {
    weekday: "long",
    day: "numeric",
    month: "long"
    ...
    className: "text-xs font-semibold",
    style: {
      color: clipState === "sync" ? C.green : C.dim
    }
  }, clipState === "sync" ? "💾 sauvegarde copiée" : "💾 …"), current && tab !== "seance" && /*#__PURE__*/React.createElement("button", {
    ...
    className: "pl-hero-glow"
  }), /*#__PURE__*/React.createElement("span", {
    className: "pl-hero-plate"
  }), /*#__PURE__*/React.createElement("div", {
    className: "text-xs font-extrabold tracking-widest uppercase",
    style: {
      opacity: .6
    }
  }, "Pr\xEAt \xE0 charger"), /*#__PURE__*/React.createElement("div", {
    className: "text-3xl leading-none mt-1",
    style: DISPLAY
  }, "D\xE9marrer une s\xE9ance"), /*#__PURE__*/React.createElement("div", {
    ...
    size: 13
  }), data.prochaine ? `${data.prochaine.nom} t'attend` : "C'est le moment")) : /*#__PURE__*/React.createElement("button", {
    onClick: () => setTab("seance"),
    className: "pl-hero pl-tap w-full rounded-3xl px-5 py-5",
    style: {
      color: "#141200"
    ...
    className: "text-xs font-extrabold tracking-widest uppercase flex items-center gap-1.5",
    style: {
      opacity: .62
    }
  }, /*#__PURE__*/React.createElement("span", {
  ...
  }), " S\xE9ance en cours"), /*#__PURE__*/React.createElement("div", {
    className: "text-3xl leading-none mt-1",
    style: DISPLAY
  }, "Reprendre \xB7 ", current.nom), /*#__PURE__*/React.createElement("div", {
    ...
    value: seancesSemaine,
    max: objSeances,
    color: seancesSemaine >= objSeances ? C.green : C.yellow
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, seancesSemaine), /*#__PURE__*/React.createElement("div", {
    className: "text-xs leading-none mt-0.5",
    style: {
      color: C.dim
    }
  }, "/ ", objSeances)), /*#__PURE__*/React.createElement("div", {
    className: "flex-1 min-w-0"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      color: seancesSemaine >= objSeances ? C.green : C.dim
    }
    ...
    className: "text-sm font-bold"
  }, "Avant de partir"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-0.5",
    style: {
      color: checklistCoches.length === checklistItems.length ? C.green : C.dim
    }
  }, checklistCoches.length, "/", checklistItems.length, " pr\xEAt", checklistCoches.length > 1 ? "s" : "", checklistCoches.length === checklistItems.length ? " — tout est bon 👍" : "")), checklistOuverte ? /*#__PURE__*/React.createElement(ChevronUp, {
    size: 16,
    style: {
      color: C.dim
    }
  ...
  })), checklistOuverte && /*#__PURE__*/React.createElement("div", {
    className: "mt-3"
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      className: "text-sm",
      style: {
        color: coche ? C.dim : C.text,
        textDecoration: coche ? "line-through" : "none"
      }
    }, item));
  }), /*#__PURE__*/React.createElement("button", {
    onClick: editerChecklist,
    ...
    return /*#__PURE__*/React.createElement(Card, {
      style: {
        border: `1px solid ${repos ? C.hair : "rgba(245,197,24,.3)"}`,
        background: repos ? undefined : "linear-gradient(165deg, rgba(245,197,24,.10), #fffdf7 62%)"
      }
    }, /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs font-extrabold tracking-widest uppercase",
      style: {
        color: repos ? C.dim : C.yellowDim
      }
    }, repos ? "Aujourd'hui" : "Ta séance du jour"), /*#__PURE__*/React.createElement("div", {
      className: "text-2xl leading-none mt-1",
      style: {
        ...DISPLAY,
        color: C.text
      }
    }, reco.titre.toUpperCase()), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1.5 leading-relaxed",
      style: {
        color: C.dim
      }
    }, reco.urgence ? /*#__PURE__*/React.createElement("b", {
      style: {
        color: C.yellowDim
      }
    }, reco.urgence, " ") : null, reco.raison))), !repos && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setStartNom(reco.titre);
        setShowStart(true);
      },
  ...
  }, data.programmesPerso.map(prog => /*#__PURE__*/React.createElement("div", {
    key: prog.id,
  ...
  }, prog.nom), /*#__PURE__*/React.createElement("div", {
    className: "text-xs",
    style: {
      color: C.dim
    }
  }, prog.exos.length, " exercice", prog.exos.length > 1 ? "s" : "")), /*#__PURE__*/React.createElement("button", {
    onClick: () => demarrerProgramme(prog),
    disabled: !!current,
    className: "pl-tap px-3 py-2 rounded-xl text-xs font-bold shrink-0",
    style: {
      background: current ? C.card2 : C.yellow,
      color: current ? C.dim : "#111"
    }
  }, "D\xE9marrer"), /*#__PURE__*/React.createElement("button", {
    onClick: () => supprimerProgrammePerso(prog.id),
    "aria-label": "Supprimer le programme",
    className: "pl-tap w-8 h-8 rounded-lg flex items-center justify-center shrink-0",
    style: {
      background: C.card2,
      color: C.red
    }
  ...
  }, ["Jambes", "Push", "Pull", "Bras", "Dos-Épaules"].map(n => /*#__PURE__*/React.createElement(Chip, {
    key: n,
    active: startNom === n,
    onClick: () => setStartNom(n)
  }, n, n === "Jambes" && !jambesFaites ? " · 1ère !" : ""))), /*#__PURE__*/React.createElement("input", {
    value: startNom,
    ...
    style: {
      background: startNom.trim() ? C.yellow : C.card2,
      color: startNom.trim() ? "#111" : C.dim
    }
  }, "C'est parti"), /*#__PURE__*/React.createElement("button", {
    ...
    return /*#__PURE__*/React.createElement(Card, {
      ...
      className: "text-sm font-bold"
    }, "Cycle ", cy.cycle, " \xB7 semaine ", cy.sem, "/", CYCLE_SEMAINES), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        if (window.confirm("Repartir sur un nouveau cycle à partir d'aujourd'hui ?")) saveData({
          ...data,
          cycleDebut: todayISO()
        });
      },
    ...
    }, "Nouveau cycle")), /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-sm font-bold"
  }, "Prochaine s\xE9ance : ", data.prochaine.nom), /*#__PURE__*/React.createElement("div", {
    className: "text-xs",
    style: {
      color: planifRelatif(data.prochaine.date).retard ? C.red : C.dim
    }
  }, fmtDate(data.prochaine.date), " \xB7 ", planifRelatif(data.prochaine.date).txt)), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setPlanifOpen(true);
      setPlanifNom(data.prochaine.nom);
      setPlanifDate(data.prochaine.date);
    },
    ...
    size: 14
  }))) : !planifOpen && /*#__PURE__*/React.createElement("button", {
    ...
    size: 15
  }), " Planifier ma prochaine s\xE9ance"), planifOpen && /*#__PURE__*/React.createElement("div", {
    className: data.prochaine ? "mt-3" : ""
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 flex-wrap mb-2"
  }, ["Jambes", "Push", "Pull", "Bras", "Dos-Épaules"].map(n => /*#__PURE__*/React.createElement(Chip, {
    key: n,
    active: planifNom === n,
    onClick: () => setPlanifNom(n)
  ...
  }, data.seances.length), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1",
    style: {
      color: C.dim
    }
  }, "s\xE9ances")), /*#__PURE__*/React.createElement(Card, {
    className: "text-center",
    style: {
      padding: "0.75rem 0.25rem"
    }
  ...
  }, data.seances.reduce((a, s) => a + s.exos.reduce((x, e) => x + (e.series || 0), 0), 0)), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1",
    style: {
      color: C.dim
    }
  }, "s\xE9ries")), /*#__PURE__*/React.createElement(Card, {
    className: "text-center",
    style: {
      padding: "0.75rem 0.25rem"
    }
  ...
  }, "sem. streak")), /*#__PURE__*/React.createElement(Card, {
    className: "text-center",
    style: {
      padding: "0.75rem 0.25rem"
    }
    ...
    style: {
      ...DISPLAY,
      ...NUMS,
      color: dernierPoids ? C.yellowDim : C.dim
    }
  }, dernierPoids ? fmtKg(dernierPoids.kg) : "—"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1",
    style: {
      color: C.dim
    }
  ...
  }, "Level Up \xB7 la salle"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-1"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => saveData({
      ...data,
      passagesOffset: (data.passagesOffset || 0) - 1
    }),
  ...
  }, passages), /*#__PURE__*/React.createElement("div", {
    className: "text-xs",
    style: {
      color: C.dim
    }
  }, "passages en salle")), /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      background: C.yellow,
      width: `${Math.min(100, passages / palier.cible * 100)}%`,
  ...
  }, fmtDate(s.date), " \xB7 ", s.exos.length, " exos")), /*#__PURE__*/React.createElement(ChevronDown, {
    size: 16,
    style: {
      color: C.dim,
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u2699\uFE0F Mes donn\xE9es"), reglagesOuvert ? /*#__PURE__*/React.createElement(ChevronUp, {
    size: 14
  }) : /*#__PURE__*/React.createElement(ChevronDown, {
    size: 14
  })), reglagesOuvert && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between mb-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm"
  }, "Mes donn\xE9es"), /*#__PURE__*/React.createElement("button", {
    onClick: () => saveData({
      ...data,
      prefAutoCopie: data.prefAutoCopie === true ? false : true
    }),
  ...
  }, "\u2600\uFE0F Mode plein soleil ", data.modeSoleil === true ? "ON" : "OFF", " \u2014 contraste boost\xE9 pour lire dehors"), /*#__PURE__*/React.createElement("button", {
    onClick: () => saveData({
      ...data,
      coachVocal: data.coachVocal === true ? false : true
    }),
    className: "w-full rounded-xl py-3 mb-3 font-semibold text-sm flex items-center justify-center gap-2",
    style: {
  ...
  }, "\uD83D\uDD0A Coach vocal ", data.coachVocal === true ? "ON" : "OFF", " \u2014 annonce la fin du repos \xE0 voix haute"), doublonsExistants.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl p-3 mb-3",
    style: {
      background: C.card,
      border: `1px solid ${C.yellowDim}`
    }
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-xs font-bold mb-2"
  }, "Exos qui se ressemblent (", doublonsExistants.length, ")"), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col gap-2"
  }, doublonsExistants.map(({
    a,
    b
  }) => /*#__PURE__*/React.createElement("div", {
    key: a + "|" + b,
  ...
  }, "Fusionner"))))), /*#__PURE__*/React.createElement("button", {
    onClick: copierContexte,
    className: "w-full rounded-xl py-3 mb-2 font-semibold text-sm flex items-center justify-center gap-2",
    style: {
      background: C.card2,
      border: `1px solid ${ctxCopie ? C.green : C.line}`,
      color: ctxCopie ? C.green : C.dim
    }
  }, /*#__PURE__*/React.createElement(Copy, {
    size: 14
  }), " ", ctxCopie ? "Copié — colle-le dans Claude" : "Copier mon contexte pour Claude"), /*#__PURE__*/React.createElement("button", {
    ...
    size: 14
  }), " Reprendre ma sauvegarde (presse-papier)"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: exporter,
    ...
    style: {
      background: C.card2,
      border: `1px solid ${copieOk ? C.green : C.line}`,
      color: copieOk ? C.green : C.text
    }
  }, /*#__PURE__*/React.createElement(Copy, {
    size: 14
  }), " ", copieOk ? "Copié !" : "Copier le JSON"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\uD83D\uDD12 Sauvegarde chiffr\xE9e par mot de passe"), exportText && /*#__PURE__*/React.createElement("textarea", {
    readOnly: true,
    value: exportText,
  ...
  }), importOpen && /*#__PURE__*/React.createElement("div", {
    className: "mt-2"
  }, /*#__PURE__*/React.createElement("textarea", {
    value: importText,
    onChange: e => setImportText(e.target.value),
    placeholder: "Colle ici un JSON export\xE9 depuis PLATEAU\u2026",
    ...
    style: {
      background: importText.trim() ? C.yellow : C.card2,
      color: importText.trim() ? "#111" : C.dim
    }
  }, "Restaurer ces donn\xE9es")), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-2",
    style: {
      color: C.dim
    }
  ...
  }, "Aucune s\xE9ance en cours."), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setTab("accueil");
      setShowStart(true);
    },
  ...
  }, "D\xE9marrer une s\xE9ance")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, {
    className: "flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "text-xl leading-none",
    style: DISPLAY
  }, current.nom.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1",
    style: {
      ...NUMS,
      color: C.dim
    ...
    return reste > 0 ? ` · reste ~${reste} min sur ${cible}` : ` · objectif ${cible} min atteint`;
  })())), /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => saveCurrent({
      ...current,
      gourdes: Math.max(0, current.gourdes - 1)
    }),
    ...
    style: NUMS
  }, current.gourdes)), /*#__PURE__*/React.createElement("button", {
    onClick: () => saveCurrent({
      ...current,
      gourdes: current.gourdes + 1
    }),
    className: "w-9 h-9 rounded-xl flex items-center justify-center",
    style: {
      background: C.yellow,
  ...
  }, "\uD83C\uDFB5 Playlist d'entra\xEEnement \u2192"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setHiitOuvert(true),
    className: "pl-touch text-xs font-semibold",
    style: {
      color: C.yellowDim
    }
  ...
  }, "Semaine all\xE9g\xE9e : charges propos\xE9es \xE0 \u221210 % et 3 s\xE9ries. C'est voulu, ne force pas."), current.template && current.template.length > 0 && !ec && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2 mb-2"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-base"
  }, "\uD83D\uDCCB"), /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm"
  }, "Programme \xB7 ", current.nom)), current.template.map((p, i) => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        background: fait ? C.green : C.card2,
        border: `1px solid ${fait ? C.green : C.line}`
      }
    }, fait && /*#__PURE__*/React.createElement(Check, {
      size: 12,
      color: "#111"
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    ...
    }, p.series, "\xD7", p.reps, " @ ", fmtKg(p.poids), " kg", p.parBras ? "/bras" : "")), !fait && /*#__PURE__*/React.createElement("div", {
      className: "text-xs font-bold shrink-0",
      style: {
        color: C.yellowDim
      }
    }, "Pr\xE9parer"));
  })), current.exos.length === 0 && !ec && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm mb-2"
  }, "\xC9chauffement \xB7 avant de charger"), echauffementPourSeance(current.nom).map((item, i) => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      className: "w-5 h-5 rounded-md flex items-center justify-center shrink-0",
      style: {
        background: done ? C.green : C.card2,
  ...
  }, "5-8 minutes qui prot\xE8gent tes articulations \u2014 surtout un jour de premi\xE8re.")), !planSeance && !ec && /*#__PURE__*/React.createElement(Card, null, current.planIA ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm mb-2"
  }, "Plan du coach \xB7 premi\xE8re s\xE9ance ", current.nom), current.planIA.map(p => {
    const fait = current.exos.some(x => x.nom.toLowerCase() === p.nom.toLowerCase());
    return /*#__PURE__*/React.createElement("button", {
      key: p.id,
    ...
    }, p.series, "\xD7", p.reps, " @ ", fmtKg(p.poids), " kg", p.parBras ? "/bras" : "", " \xB7 repos ", fmtRepos(p.reposSec)), p.conseil && /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-0.5 italic",
      style: {
        color: C.dim
      }
    }, p.conseil)), !fait && /*#__PURE__*/React.createElement("div", {
      className: "text-xs font-bold shrink-0",
      style: {
        color: C.yellowDim
      }
  ...
  }, "Charges de d\xE9part prudentes \u2014 ajuste au ressenti, c'est fait pour.")) : (() => {
    const prog = programmePour(current.nom);
    if (progVu && prog) {
      return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        className: "font-bold text-sm mb-1"
      }, "Programme type \xB7 ", prog.label), /*#__PURE__*/React.createElement("div", {
        className: "text-xs mb-3",
        style: {
          color: C.dim
        }
      }, "Charges l\xE9g\xE8res de d\xE9couverte \u2014 ajuste vite au ressenti, ce n'est qu'un point de d\xE9part."), prog.exos.map((p, i) => {
        const fait = current.exos.some(x => x.nom.toLowerCase() === p.nom.toLowerCase());
        return /*#__PURE__*/React.createElement("button", {
          key: i,
          onClick: () => !fait && pickExoPlan(p),
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "font-bold text-sm mb-1"
    }, "Premi\xE8re s\xE9ance ", current.nom, " !"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mb-3",
      style: {
        color: C.dim
      }
    }, "Aucun historique pour ce type de s\xE9ance."), /*#__PURE__*/React.createElement("div", {
      className: "flex gap-2"
    ...
    }, "Programme type"), /*#__PURE__*/React.createElement("button", {
      onClick: copierContexte,
      className: "flex-1 rounded-xl py-3 font-bold text-sm flex items-center justify-center gap-2",
      style: {
        background: C.card2,
        border: `1px solid ${C.line}`,
        color: ctxCopie ? C.green : C.text
      }
    }, /*#__PURE__*/React.createElement(Copy, {
      size: 14
    }), " ", ctxCopie ? "Copié !" : "Demander à Claude")));
  })()), planSeance && !ec && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("button", {
    className: "w-full flex items-center justify-between",
    onClick: () => setPlanOuvert(!planOuvert)
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm"
  }, "Plan \xB7 d'apr\xE8s ta derni\xE8re s\xE9ance ", current.nom), planOuvert ? /*#__PURE__*/React.createElement(ChevronUp, {
    size: 16,
    style: {
      color: C.dim
    }
  ...
  })), planOuvert && /*#__PURE__*/React.createElement("div", {
    className: "mt-2"
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-sm font-semibold truncate"
    }, p.nom), s2 && !fait && /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        ...NUMS,
  ...
  }, "Les fl\xE8ches r\xE9ordonnent le plan \u2014 l'ordre est retenu pour tes prochaines s\xE9ances ", current.nom, "."))), ec && /*#__PURE__*/React.createElement(Card, {
    style: {
      border: `1px solid ${C.yellow}`
    }
  }, /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => setFocusOuvert(true),
    className: "text-xs font-bold",
    style: {
      color: C.yellowDim
    }
  }, "Plein \xE9cran"), /*#__PURE__*/React.createElement("button", {
    onClick: () => abandonnerExo(false),
    className: "text-xs font-semibold",
    style: {
      color: C.red
    }
  }, "Abandonner"))), /*#__PURE__*/React.createElement("div", {
    className: "text-sm mb-3",
    style: {
      color: C.dim
    }
  }, "S\xE9rie ", ec.sets.length, ec.prevu ? `/${ec.prevu}` : "", " faite \xB7 repos ", fmtRepos(ec.reposSec), ec.parBras ? " · par bras" : ""), ec.sets.map((s, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    ...
    style: NUMS
  }, fmtKg(s.poids), " kg \xD7 ", s.reps), /*#__PURE__*/React.createElement("button", {
  ...
  }, "Ressenti global de l'exo"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "De la marge"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\xC7a tirait")), /*#__PURE__*/React.createElement("button", {
    ...
    size: 18
  }), " S\xE9rie ", ec.sets.length + 1, " faite \u2192 chrono"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Derni\xE8re s\xE9rie \u2192 fin de l'exo"), /*#__PURE__*/React.createElement("button", {
    onClick: finirExo,
  ...
  }, "Finir sans", /*#__PURE__*/React.createElement("br", null), "ajouter")), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-2 text-center",
    style: {
      color: C.dim
    }
  }, ec.sets.length, " s\xE9rie", ec.sets.length > 1 ? "s" : "", " enregistr\xE9e", ec.sets.length > 1 ? "s" : "", " \u2014 la s\xE9rie en cours n'est compt\xE9e que si tu la valides.")), !ec && /*#__PURE__*/React.createElement(Card, {
    style: editExoId ? {
      border: `1px solid ${C.yellow}`
    } : undefined
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "font-bold text-sm"
  }, editExoId ? "Modifier l'exercice" : "Nouvel exercice"), editExoId ? /*#__PURE__*/React.createElement("button", {
    onClick: resetForm,
    className: "text-xs font-semibold",
    style: {
      color: C.dim
    }
  }, "Annuler la modif") : /*#__PURE__*/React.createElement("button", {
    onClick: () => saveData({
      ...data,
      prefSerieParSerie: !serieMode
    }),
    className: "px-2 py-1 rounded-lg text-xs font-bold",
    style: {
  ...
  }, "S\xE9rie par s\xE9rie ", serieMode ? "ON" : "OFF")), /*#__PURE__*/React.createElement("input", {
    value: fNom,
  ...
  }), !editExoId && /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 overflow-x-auto pb-2 mb-2",
    style: {
      scrollbarWidth: "none"
    }
  }, suggestionsNoms.map(n => /*#__PURE__*/React.createElement(Chip, {
    key: n,
    active: fNom === n,
    onClick: () => pickExo(n)
  }, n))), fNom.trim() && !isCardio && !editExoId && /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl p-2.5 mb-3 flex items-center gap-3",
    style: {
      background: C.card2,
      border: `1px solid ${C.hair}`
    }
  }, rappelPhotoId ? /*#__PURE__*/React.createElement(PhotoThumb, {
    photoId: rappelPhotoId,
    onView: setPhotoView,
    size: 56
  }) : /*#__PURE__*/React.createElement("button", {
  ...
  }, rappelPhotoId ? "Ta machine" : "Pas encore de photo"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs",
    style: {
      color: C.dim
    }
  }, rappelPhotoId ? "Tape pour l'agrandir" : "Prends-la en photo une fois")), /*#__PURE__*/React.createElement("a", {
  ...
  }, "\uD83C\uDFA5 Vid\xE9o")), fNom.trim() && !isCardio && !editExoId && (data.notesExo || {})[fNom.trim().toLowerCase()] && /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl px-3 py-2 mb-3 text-xs leading-relaxed",
    style: {
      background: C.card2,
      border: `1px solid ${C.yellowDim}`
    }
  }, "\uD83D\uDCDD ", /*#__PURE__*/React.createElement("b", null, "Ta note :"), " ", (data.notesExo || {})[fNom.trim().toLowerCase()]), sugg && !isCardio && /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl px-3 py-2 mb-3 text-xs leading-relaxed",
    style: {
      background: C.card2,
  ...
  }, "Ressemble \xE0 ", /*#__PURE__*/React.createElement("b", null, doublonProbable.nom), " que tu fais d\xE9j\xE0.", " ", /*#__PURE__*/React.createElement("button", {
    onClick: () => pickExo(doublonProbable.nom),
    className: "font-bold underline",
    style: {
      color: C.yellowDim
    }
  }, "C'est le m\xEAme"), /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.dim
    }
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs font-bold"
    }, "Rep\xE8re \xB7 ", fmtKg(poidsCorps), " kg de corps"), /*#__PURE__*/React.createElement("span", {
    ...
    }, /*#__PURE__*/React.createElement("span", null, "d\xE9b ", fmtKg(rp.paliers[0])), /*#__PURE__*/React.createElement("span", null, "nov ", fmtKg(rp.paliers[1])), /*#__PURE__*/React.createElement("span", null, "int ", fmtKg(rp.paliers[2])), /*#__PURE__*/React.createElement("span", null, "av ", fmtKg(rp.paliers[3]))), charge > 0 && manque != null && /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1 font-semibold",
      style: {
        color: niv.color
      }
    }, "Encore ", fmtKg(manque), " kg pour le rep\xE8re d\xE9butant \u2014 \xE7a viendra."), charge > 0 && prochain != null && /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1",
      style: {
        color: C.dim,
        ...NUMS
      }
    }, "Prochain palier \xE0 ", fmtKg(prochain), " kg", fParBras ? " au total (2 bras)" : "", "."), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1",
      style: {
        color: C.dim
      }
    }, rp.machine ? "Indicatif : les charges machine varient beaucoup d'une marque à l'autre." : "Repère standard pour un homme de adulte, charge totale."));
  })(), fNom.trim() && !isCardio && /*#__PURE__*/React.createElement("div", {
    className: "mb-3"
  }, /*#__PURE__*/React.createElement("button", {
    ...
    className: "text-xs font-bold"
  }, "Sch\xE9ma & technique"), schemaOuvert ? /*#__PURE__*/React.createElement(ChevronUp, {
    size: 14,
    style: {
      color: C.dim
    }
  ...
  })), schemaOuvert && /*#__PURE__*/React.createElement("div", {
    className: "rounded-xl mt-2 p-3",
    style: {
      background: C.card2,
      border: `1px solid ${C.line}`
    }
  }, /*#__PURE__*/React.createElement(SchemaMouvement, {
    nom: fNom
  }), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1 mb-2 text-center",
    style: {
      color: C.dim
    }
  }, "trait plein = d\xE9part \xB7 pointill\xE9s = arriv\xE9e"), (TECHNIQUE[mouvementOf(fNom)] || TECHNIQUE.generique).map((t, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
  ...
  }, "\xB7"), " ", t)), /*#__PURE__*/React.createElement("a", {
    href: `https://www.youtube.com/results?search_query=${encodeURIComponent(fNom.trim() + " technique musculation")}`,
    target: "_blank",
    rel: "noreferrer",
  ...
  }, "Voir une d\xE9mo vid\xE9o \u2192"))), !editExoId && !isCardio && fNom.trim() && !isNaN(parseFloat(String(fPoids).replace(",", "."))) && /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-3",
    style: {
      color: C.dim
    }
  }, "\xC9chauffement conseill\xE9 : ", echauffementPour(parseFloat(String(fPoids).replace(",", "."))), " \u2014 pas compt\xE9 dans tes s\xE9ries."), !editExoId && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("input", {
    ref: fileInputRef,
    type: "file",
    accept: "image/*",
    capture: "environment",
    ...
    size: 16
  }), " ", photoBusy ? "Traitement…" : fPhotoPreview ? "Reprendre" : "Photo machine"), fPhotoPreview && /*#__PURE__*/React.createElement("div", {
    className: "relative"
  }, /*#__PURE__*/React.createElement("img", {
    src: fPhotoPreview,
    alt: "Machine",
    ...
    active: !isCardio,
    onClick: () => setFTypeManuel("muscu")
  }, "Muscu"), /*#__PURE__*/React.createElement(Chip, {
    active: isCardio,
    onClick: () => setFTypeManuel("cardio")
  }, "Cardio")), isCardio ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Niveau"), /*#__PURE__*/React.createElement("input", {
    value: fNiveau,
    ...
    className: "flex gap-2 mb-3"
  }, [5, 10, 15, 20, 30].map(m => /*#__PURE__*/React.createElement(Chip, {
    key: m,
    active: fDuree === m,
    onClick: () => setFDuree(m)
  ...
  }, "Ressenti"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Facile"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\xC7a piquait")), /*#__PURE__*/React.createElement("button", {
    onClick: validerExo,
    disabled: !fNom.trim(),
    className: "w-full rounded-xl py-4 text-lg font-black flex items-center justify-center gap-2",
    style: {
      background: fNom.trim() ? C.yellow : C.card2,
      color: fNom.trim() ? "#111" : C.dim
    }
  }, /*#__PURE__*/React.createElement(Check, {
    size: 20
  }), " ", editExoId ? "Enregistrer la modif" : "Valider le cardio")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Poids (kg)"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\uD83E\uDDEE Calcul des plaques ", plaquesOuvert ? "▴" : "▾"), plaquesOuvert && (() => {
    ...
    }, "Barre :"), [15, 20].map(b => /*#__PURE__*/React.createElement(Chip, {
      key: b,
      active: barreKg === b,
      onClick: () => setBarreKg(b)
    }, b, " kg"))), r.parCote.length === 0 ? /*#__PURE__*/React.createElement("div", {
      className: "text-sm",
      style: {
        color: C.dim
      }
    }, "Juste la barre (", barreKg, " kg) suffit.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-1"
    }, "Par c\xF4t\xE9 : ", r.parCote.map(p => fmtKg(p)).join(" + "), " kg"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim,
        ...NUMS
      }
    }, "Total r\xE9el : ", fmtKg(r.atteint), " kg", r.reste > 0 && ` (visé ${fmtKg(cible)} kg, écart ${fmtKg(r.reste * 2)} kg — pas de plaque assez fine)`)));
  })()), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-1",
    style: {
      color: C.dim
    ...
    className: "flex gap-2 mb-3"
  }, [60, 90, 120, 150].map(s => /*#__PURE__*/React.createElement(Chip, {
    key: s,
    active: fRepos === s,
    onClick: () => setFRepos(s)
  }, fmtRepos(s)))), (!serieMode || editExoId) && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-1",
    style: {
      color: C.dim
    }
  ...
  }, "\xC7a tirait")), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-1",
    style: {
      color: C.dim
    }
  }, "RPE (optionnel) \u2014 effort ressenti sur 10"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-1 mb-4"
  }, [6, 7, 8, 9, 10].map(r => /*#__PURE__*/React.createElement("button", {
    key: r,
  ...
  }, "Tempo (optionnel, ex: 3-1-1)"), /*#__PURE__*/React.createElement("input", {
    value: fTempo,
    onChange: ev => setFTempo(ev.target.value),
    placeholder: "descente-pause-mont\xE9e",
  ...
  }), current && current.exos.length > 0 && /*#__PURE__*/React.createElement("button", {
  ...
  }, "\uD83D\uDD17 Encha\xEEn\xE9 avec ", current.exos[current.exos.length - 1].nom, " (sans repos)")), /*#__PURE__*/React.createElement("button", {
    onClick: validerExo,
    ...
    size: 20
  }), editExoId ? "Enregistrer la modif" : serieMode ? "Série 1 faite → chrono" : "Valider + chrono"))), current.exos.length > 0 && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between mb-1"
  }, /*#__PURE__*/React.createElement("div", {
    className: "font-bold text-sm"
  }, "Valid\xE9 aujourd'hui (", current.exos.length, ")"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs font-bold",
    style: {
      ...NUMS,
    ...
    ressenti: e.ressenti
  }), e.nom, e.pr && /*#__PURE__*/React.createElement(PrBadge, null)), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-0.5",
    style: {
      ...NUMS,
      color: C.dim
    }
  }, e.type === "cardio" ? `${e.dureeCardio} min${e.distance ? ` · ${fmtKg(e.distance)} km` : ""}${e.niveau ? ` · niveau ${e.niveau}` : ""}` : `${e.series}×${e.reps} @ ${fmtKg(e.poids)} kg${e.parBras ? "/bras" : ""} · repos ${fmtRepos(e.reposSec)}${e.dureeMin ? ` · ${e.dureeMin} min` : ""}${e.rpe ? ` · RPE ${e.rpe}` : ""}`), e.type !== "cardio" && (() => {
    ...
    }, "premi\xE8re fois sur cet exo");
    return /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-0.5 font-bold",
      style: {
        ...NUMS,
        color: d.delta > 0 ? C.green : d.delta < 0 ? C.red : C.dim
      }
    }, d.delta > 0 ? `+${fmtKg(d.delta)} kg` : d.delta < 0 ? `${fmtKg(d.delta)} kg` : "même charge", /*#__PURE__*/React.createElement("span", {
      style: {
        color: C.dim,
        fontWeight: 400
      }
    }, " vs ", fmtKg(d.avant), " kg le ", fmtShort(d.date)));
  ...
  }, e.note), coachBusyId === e.id && /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1 italic",
    style: {
      color: C.dim
    }
  }, "Le coach r\xE9fl\xE9chit\u2026"), e.coach && /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1 leading-relaxed",
    style: {
      color: C.yellowDim
    }
  }, e.coach)), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Note de s\xE9ance"), /*#__PURE__*/React.createElement("textarea", {
    value: current.note || "",
  ...
  })), finOuvert && /*#__PURE__*/React.createElement(Card, {
    style: {
      border: `1px solid ${C.yellow}`
    }
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold mb-1"
  }, "Combien de gourdes bues ?"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-3",
    style: {
      color: C.dim
    }
  }, "Tu oublies souvent de les compter pendant l'effort \u2014 c'est le moment."), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 flex-wrap mb-3"
  }, [0, 1, 2, 3, 4, 5].map(g => /*#__PURE__*/React.createElement(Chip, {
    key: g,
    active: current.gourdes === g,
    onClick: () => saveCurrent({
      ...current,
      gourdes: g
    })
  ...
  }, "Terminer la s\xE9ance"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "Continuer la s\xE9ance")), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    ...
    size: 18
  }))))), tab === "recup" && /*#__PURE__*/React.createElement("div", {
    className: "space-y-3 pl-anim"
  }, /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm mb-2 flex items-center gap-2 font-bold"
    ...
    className: "px-6 py-5",
    onClick: () => setJour({
      sommeil: Math.max(0, (jourDuJour.sommeil ?? 7.5) - 0.5)
    }),
    style: {
      color: C.text
    }
    ...
    style: {
      ...NUMS,
      color: jourDuJour.sommeil != null ? C.text : C.dim
    }
  }, jourDuJour.sommeil != null ? `${fmtKg(jourDuJour.sommeil)} h` : "—"), /*#__PURE__*/React.createElement("button", {
    className: "px-6 py-5",
    onClick: () => setJour({
      sommeil: Math.min(14, (jourDuJour.sommeil ?? 6.5) + 0.5)
    }),
    style: {
      color: C.text
    }
    ...
    size: 20
  }))), sommeilMoyen != null && /*#__PURE__*/React.createElement("div", {
  ...
  }), " Eau bue"), /*#__PURE__*/React.createElement("div", {
    ...
    className: "px-6 py-4",
    onClick: () => setJour({
      eau: Math.max(0, Math.round(((jourDuJour.eau || 0) - 0.25) * 100) / 100)
    }),
    style: {
      color: C.text
    }
    ...
    style: {
      ...NUMS,
      color: jourDuJour.eau ? C.text : C.dim
    }
  }, jourDuJour.eau ? `${fmtKg(jourDuJour.eau)} L` : "—"), jourDuJour.eau > 0 && /*#__PURE__*/React.createElement("div", {
    className: "text-xs",
    style: {
      color: jourDuJour.eau >= 2 ? C.green : C.dim
    }
  }, jourDuJour.eau >= 2 ? "objectif 2 L atteint" : `reste ${fmtKg(2 - jourDuJour.eau)} L`)), /*#__PURE__*/React.createElement("button", {
    className: "px-6 py-4",
    onClick: () => setJour({
      eau: Math.round(((jourDuJour.eau || 0) + 0.25) * 100) / 100
    }),
    style: {
      color: C.text
    }
    ...
    label: "Douche",
    Ic: ShowerHead,
    hint: seanceAujourdhui ? "après la séance, priorité peau" : null
  }, {
    k: "dentsMatin",
    label: "Dents · matin",
    Ic: Sparkles
    ...
    k: "visageMatin",
    label: "Soins visage · matin",
    Ic: Droplets
  }, {
    k: "visageSoir",
    label: "Soins visage · soir",
    Ic: Droplets
  }].map(({
    k,
    label,
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "Routine du jour"), /*#__PURE__*/React.createElement("span", {
      className: "font-bold",
      style: {
        ...NUMS,
        color: faits === total ? C.green : C.dim
      }
    }, faits, "/", total)), /*#__PURE__*/React.createElement("div", {
      className: "h-2 rounded-full overflow-hidden",
      style: {
        background: C.card2
      }
      ...
      className: "h-full rounded-full",
      style: {
        width: `${faits / total * 100}%`,
        background: faits === total ? C.green : C.yellow,
        transition: "width .3s"
      }
    })));
  })()), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    ...
    const ok = k === "dents" ? j.dentsMatin && j.dentsSoir : k === "visage" ? j.visageMatin && j.visageSoir : k === "eau" ? (j.eau || 0) >= 2 : !!j[k];
    const partiel = k === "dents" ? j.dentsMatin || j.dentsSoir : k === "visage" ? j.visageMatin || j.visageSoir : k === "eau" ? (j.eau || 0) > 0 : false;
    return /*#__PURE__*/React.createElement("div", {
      key: i,
      className: "flex-1 rounded",
      style: {
        height: 26,
        background: ok ? C.green : partiel ? C.yellowDim : C.card2,
        border: `1px solid ${ok ? C.green : C.line}`
      }
    });
  })))), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-2",
  ...
  }, "7 derniers jours, du plus ancien \xE0 aujourd'hui \u2014 vert = complet, jaune = partiel.")), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm mb-2 flex items-center gap-2 font-bold"
  ...
  }), " Poids de corps", /*#__PURE__*/React.createElement("span", {
    className: "font-normal text-xs",
    style: {
      color: C.dim
    }
  }, "objectif ", OBJECTIF_POIDS, " kg")), dernierPoids && /*#__PURE__*/React.createElement("div", {
  ...
  }, fmtKg(dernierPoids.kg), " kg", /*#__PURE__*/React.createElement("span", {
    className: "text-sm ml-2",
    style: {
      color: C.dim,
      fontFamily: "inherit"
    }
  }, joursDepuis(dernierPoids.date) === 0 ? "aujourd'hui" : `il y a ${joursDepuis(dernierPoids.date)} j`, OBJECTIF_POIDS - dernierPoids.kg > 0 ? ` · encore ${fmtKg(OBJECTIF_POIDS - dernierPoids.kg)} kg` : " · atteint !")), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2"
  }, /*#__PURE__*/React.createElement("input", {
    value: pdsInput,
    onChange: e => setPdsInput(e.target.value),
    inputMode: "decimal",
    placeholder: "Nouvelle pes\xE9e (kg)\u2026",
    ...
    style: {
      background: pdsInput.trim() ? C.yellow : C.card2,
      color: pdsInput.trim() ? "#111" : C.dim
    }
  }, "OK"))), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold mb-2"
  }, "Mon rythme"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-1.5 mb-2"
  }, [...Array(7)].map((_, i) => {
    const dt = new Date();
    dt.setDate(dt.getDate() - (6 - i));
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        height: 34,
        background: seance ? C.yellow : C.card2,
        border: `1px solid ${seance ? C.yellow : C.line}`
      }
    }), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
    ...
    }, ["L", "M", "M", "J", "V", "S", "D"][(dt.getDay() + 6) % 7]));
  })), /*#__PURE__*/React.createElement("div", {
    className: "text-sm",
    style: {
      color: C.dim
    }
  }, reposDepuis == null ? "Aucune séance enregistrée." : reposDepuis === 0 ? "Séance aujourd'hui — pense à la douche." : `Dernière séance il y a ${reposDepuis} j.`)), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("button", {
    onClick: () => setRecupPlus(!recupPlus),
    className: "w-full flex items-center justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-sm font-bold"
  }, "Mensurations"), recupPlus ? /*#__PURE__*/React.createElement(ChevronUp, {
    size: 16,
    style: {
      color: C.dim
    }
  ...
  })), recupPlus && /*#__PURE__*/React.createElement("div", {
    className: "mt-3"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, zone, " : "), last ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("b", null, fmtKg(last.cm), " cm"), delta != null && delta !== 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      color: delta > 0 ? C.green : C.red
    }
  }, " (", delta > 0 ? "+" : "", fmtKg(delta), ")")) : /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.line
    }
  }, "\u2014")))), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 mb-2"
  }, ZONES.map(z => /*#__PURE__*/React.createElement(Chip, {
    key: z,
    active: mensuZone === z,
    onClick: () => setMensuZone(z)
    ...
    className: "px-6 rounded-2xl font-black",
    style: {
      background: mensuInput.trim() ? C.yellow : C.card2,
      color: mensuInput.trim() ? "#111" : C.dim
    }
  }, "OK")), (data.mensu || []).filter(m => m.zone === mensuZone).length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "mt-2"
  }, [...(data.mensu || [])].filter(m => m.zone === mensuZone).reverse().slice(0, 5).map(m => /*#__PURE__*/React.createElement("div", {
    key: m.date + m.zone,
    className: "flex items-center gap-2 py-2",
    style: {
      borderTop: `1px solid ${C.line}`
  ...
  }, fmtShort(m.date)), /*#__PURE__*/React.createElement("div", {
    className: "text-sm flex-1 font-semibold",
    style: NUMS
  }, fmtKg(m.cm), " cm"), /*#__PURE__*/React.createElement("button", {
    onClick: () => supprimerMensu(m.date, m.zone),
  ...
  }, fmtShort(pp.date)), /*#__PURE__*/React.createElement("div", {
    className: "text-sm flex-1 font-semibold",
    style: NUMS
  }, fmtKg(pp.kg), " kg"), /*#__PURE__*/React.createElement("button", {
    onClick: () => supprimerPoids(pp.date),
    className: "w-7 h-7 rounded-md flex items-center justify-center shrink-0",
    style: {
      background: C.card2,
      color: C.red
    ...
    return /*#__PURE__*/React.createElement(Card, {
    ...
    }, fmtDate(s.date), s.duree ? ` · ${fmtDuree(s.duree)}` : "", s.gourdes != null ? ` · ${s.gourdes} 🥤` : "", ` · ${s.exos.length} exos`, (() => {
      const kcal = caloriesSeance(s, poidsCorps);
      return kcal ? ` · ~${kcal} kcal` : "";
    })())), open ? /*#__PURE__*/React.createElement(ChevronUp, {
      size: 18,
      style: {
        color: C.dim
      }
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => setFicheExo(e.nom),
        className: "text-sm font-semibold text-left"
      }, /*#__PURE__*/React.createElement(Dot, {
        ressenti: e.ressenti
      }), e.nom, e.pr && /*#__PURE__*/React.createElement(PrBadge, null)), !enEdition && /*#__PURE__*/React.createElement("div", {
        className: "text-xs mt-0.5",
        style: {
          ...NUMS,
          color: C.dim
        }
      }, e.type === "cardio" ? `${e.dureeCardio} min${e.distance ? ` · ${fmtKg(e.distance)} km` : ""}${e.niveau ? ` · niveau ${e.niveau}` : ""}` : `${e.series}×${e.reps} @ ${fmtKg(e.poids)} kg${e.parBras ? "/bras" : ""}${e.reposSec ? ` · repos ${fmtRepos(e.reposSec)}` : ""}${e.dureeMin ? ` · ${e.dureeMin} min` : ""}`, e.tempo ? ` · tempo ${e.tempo}` : "", e.note ? ` · ${e.note}` : ""), !enEdition && e.superset && /*#__PURE__*/React.createElement("div", {
        className: "text-xs mt-0.5",
        style: {
          color: C.yellowDim
        }
      }, "\uD83D\uDD17 encha\xEEn\xE9 sans repos"), !enEdition && e.coach && /*#__PURE__*/React.createElement("div", {
        className: "text-xs mt-1",
        style: {
          color: C.yellowDim
        }
        ...
        style: {
          background: C.card2,
          color: enEdition ? C.yellowDim : C.dim
        }
      }, /*#__PURE__*/React.createElement(Pencil, {
        size: 12
      })), /*#__PURE__*/React.createElement("button", {
        ...
        size: 12
      }))))), enEdition && /*#__PURE__*/React.createElement("div", {
        className: "flex items-end gap-2 mt-2"
      }, ehCardio ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Stepper, {
        label: "Dur\xE9e (min)",
        value: ehDuree,
        onChange: setEhDuree,
        min: 1,
      ...
      }, "Poids (kg)"), /*#__PURE__*/React.createElement("input", {
        value: ehPoids,
    ...
    }), (s.note || ehNoteKey === s.id) && /*#__PURE__*/React.createElement("div", {
    ...
    }, "Ta note"), ehNoteKey === s.id ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("textarea", {
      value: ehNote,
      ...
      className: "text-sm leading-relaxed"
    }, s.note)), bilanBusyId === s.id && /*#__PURE__*/React.createElement("div", {
      className: "text-xs italic mt-2",
      style: {
        color: C.dim
      }
    }, "Le coach pr\xE9pare son bilan\u2026"), s.bilan && /*#__PURE__*/React.createElement("div", {
      ...
      className: "flex items-center gap-1 mb-1 text-xs font-bold",
      style: {
        color: C.yellowDim
      }
    }, /*#__PURE__*/React.createElement(Sparkles, {
      size: 12
    }), " Bilan du coach"), s.bilan), shareFallback && shareFallback.id === s.id && /*#__PURE__*/React.createElement("textarea", {
      readOnly: true,
      ...
      style: {
        color: shareId === s.id ? C.green : C.yellowDim
      }
    }, /*#__PURE__*/React.createElement(Share2, {
      size: 12
    }), " ", shareId === s.id ? "Récap copié !" : "Partager le récap"), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setEhNoteKey(s.id);
        setEhNote(s.note || "");
      },
      ...
      size: 12
    }), " Exo"), /*#__PURE__*/React.createElement("button", {
      ...
      size: 12
    }), " Refaire"), /*#__PURE__*/React.createElement("button", {
      ...
      size: 12
    }), " Carte image"), /*#__PURE__*/React.createElement("button", {
    ...
    active: progMode === "exo",
    onClick: () => setProgMode("exo")
  }, "Exercices"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "corps",
    onClick: () => setProgMode("corps")
  }, "Corps"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "muscles",
    onClick: () => setProgMode("muscles")
  }, "Muscles"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "equilibre",
    onClick: () => setProgMode("equilibre")
  }, "\xC9quilibre"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "retro",
    onClick: () => setProgMode("retro")
  }, "R\xE9tro \u2728"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "records",
    onClick: () => setProgMode("records")
  }, "Records"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "volume",
    onClick: () => setProgMode("volume")
  }, "Volume"), /*#__PURE__*/React.createElement(Chip, {
    active: progMode === "bilan",
    onClick: () => setProgMode("bilan")
  }, "Bilan")), progMode === "exo" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-1",
    style: {
      color: C.dim
    }
  }, "Exercice"), /*#__PURE__*/React.createElement("select", {
    value: selExo,
  ...
  }, allExos.map(n => /*#__PURE__*/React.createElement("option", {
    key: n,
    value: n
  }, n)))), selExo && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between mb-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold"
  }, "Mon objectif"), objectifSel && !objEdit && /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-3"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setObjInput(fmtKg(objectifSel.cible));
      setObjEdit(true);
    },
    className: "text-xs font-bold",
    style: {
    ...
    onClick: retirerObjectif,
    className: "text-xs font-semibold",
    style: {
      color: C.red
    }
  }, "Retirer"))), objectifSel && !objEdit ? (() => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, fmtKg(rec), " ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 16,
        color: C.dim
      }
    }, "/ ", fmtKg(objectifSel.cible), " kg")), /*#__PURE__*/React.createElement("div", {
      className: "text-xs font-bold",
      style: {
        ...NUMS,
        color: reste <= 0 ? C.green : C.dim
      }
    }, reste <= 0 ? "Atteint !" : `encore ${fmtKg(reste)} kg`)), /*#__PURE__*/React.createElement("div", {
      className: "h-2 rounded-full overflow-hidden",
      style: {
        background: C.card2
      }
      ...
      style: {
        width: `${pct}%`,
        background: reste <= 0 ? C.green : C.yellow,
        transition: "width .4s"
      }
    })));
  })() : /*#__PURE__*/React.createElement("div", {
    ...
    inputMode: "decimal",
    placeholder: "Charge \xE0 atteindre (kg)\u2026",
    ...
    className: "px-4 rounded-xl font-bold text-sm",
    style: {
      background: objInput.trim() ? C.yellow : C.card2,
      color: objInput.trim() ? "#111" : C.dim
    }
  }, "OK"), objEdit && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setObjEdit(false);
      setObjInput("");
    },
    ...
    }, "1RM estim\xE9 : ", fmtKg(Math.round(rm1Sel * 2) / 2), " kg"), " ", "\u2014 la charge que tu sortirais sur une seule r\xE9p\xE9tition, calcul\xE9e depuis tes s\xE9ries. Estimation, ne la teste pas en vrai sans partenaire.", rmB != null && rmL != null && /*#__PURE__*/React.createElement("div", {
      className: "mt-1",
      style: {
        ...NUMS
      }
    }, "Epley ", fmtKg(rm1Sel), " kg \xB7 Brzycki ", fmtKg(rmB), " kg \xB7 Lombardi ", fmtKg(rmL), " kg"), ratio != null && /*#__PURE__*/React.createElement("div", {
      className: "mt-1",
      style: {
        ...NUMS
      }
    }, "Soit ", ratio.toFixed(2), "\xD7 ton poids de corps"));
  })()), /*#__PURE__*/React.createElement(Card, null, chartData.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-center py-8",
    style: {
      color: C.dim
    }
  }, "Pas encore de donn\xE9es pour cet exercice.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-between mb-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold"
  }, "Charge max par s\xE9ance ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.dim,
      fontWeight: 400
    }
  }, "(kg)")), recordExo != null && /*#__PURE__*/React.createElement("div", {
    ...
    size: 12
  }), " ", fmtKg(recordExo), " kg")), /*#__PURE__*/React.createElement(MiniLine, {
    data: chartData,
    dataKey: "poids",
    color: C.yellow,
    height: 240
  ...
  }, "Une seule s\xE9ance pour l'instant \u2014 la courbe se dessinera au fil des prochaines.")))), progMode === "corps" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, null, chartPoids.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-center py-8",
    style: {
      color: C.dim
    }
  }, "Pas encore de pes\xE9e \u2014 enregistre ton poids dans l'onglet R\xE9cup.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold mb-2"
  }, "Poids de corps ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.dim,
  ...
  }, "(objectif ", OBJECTIF_POIDS, " kg)")), /*#__PURE__*/React.createElement(MiniLine, {
    data: chartPoids,
    dataKey: "kg",
    color: C.green,
    height: 220,
    ...
    className: "text-sm font-bold mb-2"
  }, "Mensurations"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 mb-3"
  }, ZONES.map(z => /*#__PURE__*/React.createElement(Chip, {
    key: z,
    active: progMensuZone === z,
    onClick: () => setProgMensuZone(z)
  }, z))), chartMensu.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-center py-6",
    style: {
      color: C.dim
    }
  }, "Pas encore de mesure pour ", progMensuZone.toLowerCase(), " \u2014 saisis-la dans R\xE9cup.") : /*#__PURE__*/React.createElement(MiniLine, {
    data: chartMensu,
    dataKey: "cm",
    color: C.yellow,
    height: 200
  }))), progMode === "muscles" && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold mb-1"
  }, "S\xE9ries par groupe musculaire"), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mb-3",
    style: {
      color: C.dim
    }
  }, "7 derniers jours (s\xE9ance en cours incluse)"), MUSCLE_ORDRE.filter(m => m !== "Autre" || volumes[m]).map(m => {
    const v = volumes[m] || 0;
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      style: {
        color: alerte ? C.red : C.text
      }
    }, m), /*#__PURE__*/React.createElement("span", {
      style: {
        ...NUMS,
        color: atteint ? C.green : alerte ? C.red : C.dim
      }
    }, cible ? `${v} / ${cible} séries` : `${v} série${v > 1 ? "s" : ""}`)), /*#__PURE__*/React.createElement("div", {
      className: "h-2 rounded-full overflow-hidden",
      style: {
        background: C.card2
      }
      ...
      className: "h-full rounded-full",
      style: {
        background: atteint ? C.green : alerte ? C.red : C.yellow,
        width: `${pct}%`,
        transition: "width 0.4s"
      }
    })), (() => {
      const z = zoneVolume(m, v);
      return z ? /*#__PURE__*/React.createElement("div", {
        className: "text-xs mt-0.5",
        style: {
          color: z.couleur
        }
      }, z.label, " \u2014 ", z.conseil) : null;
    })());
  }), /*#__PURE__*/React.createElement("div", {
    className: "text-xs mt-1",
    style: {
  ...
  }, "Tape un groupe pour fixer sa cible de s\xE9ries hebdo. Les rep\xE8res (minimum / zone efficace / plafond) viennent des recommandations classiques en hypertrophie \u2014 indicatifs, pas une r\xE8gle absolue."), /*#__PURE__*/React.createElement("div", {
    className: "mt-4 pt-3",
    style: {
      borderTop: `1px solid ${C.line}`
    }
  ...
  }, "Un coach surveille les paires oppos\xE9es, pas seulement le volume total."), equilibres(data, current ? current.exos : []).map(e => {
    const [n1, n2] = e.label.split(" / ");
    if (e.etat === "vide") {
      return /*#__PURE__*/React.createElement("div", {
        key: e.label,
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-xs mt-1",
      style: {
        color: e.etat === "ok" ? C.dim : coul
      }
    }, e.etat === "ok" ? "équilibré" : e.etat === "tropA" ? `trop de ${n1.toLowerCase()} — rattrape le ${n2.toLowerCase()}` : `trop de ${n2.toLowerCase()} — rattrape le ${n1.toLowerCase()}`));
  })), (() => {
    const dep = new Date();
    dep.setDate(dep.getDate() - 6);
    const iso = isoOf(dep);
    const min = data.seances.filter(s => s.date >= iso).reduce((a, s) => a + s.exos.filter(e => e.type === "cardio").reduce((x, e) => x + (e.dureeCardio || 0), 0), 0) + (current ? current.exos.filter(e => e.type === "cardio").reduce((x, e) => x + (e.dureeCardio || 0), 0) : 0);
    return /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-3 pt-3 font-semibold",
      style: {
        borderTop: `1px solid ${C.line}`,
        color: min > 0 ? C.text : C.dim
      }
    }, "Cardio cette semaine : ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: min > 0 ? C.yellowDim : C.dim,
        ...NUMS
      }
    }, min, " min"));
  })()), progMode === "bilan" && (() => {
    ...
    const nomMois = now.toLocaleDateString("fr-FR", {
      month: "long",
      year: "numeric"
    });
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-3"
    }, "Bilan de ", nomMois), /*#__PURE__*/React.createElement("div", {
    ...
    }, seancesMois.length), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim
      }
    ...
    }, seriesMois), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim
      }
    }, "s\xE9ries")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "text-2xl flex items-center gap-1",
      style: {
        ...DISPLAY,
        ...NUMS,
        color: recordsMois > 0 ? C.yellowDim : C.dim
      }
    }, /*#__PURE__*/React.createElement(Trophy, {
      size: 18
    }), " ", recordsMois), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim
      }
    }, "records battus")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "text-2xl",
      style: {
        ...DISPLAY,
        ...NUMS,
        color: deltaPoids > 0 ? C.green : deltaPoids < 0 ? C.red : C.dim
      }
    }, deltaPoids != null ? `${deltaPoids > 0 ? "+" : ""}${fmtKg(deltaPoids)} kg` : "—"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim
      }
    ...
    }, "Tous les groupes travaill\xE9s ce mois \u2014 bon \xE9quilibre.")), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-1"
    }, "Depuis le 1er janvier ", anISO), /*#__PURE__*/React.createElement("div", {
      className: "text-sm",
      style: {
        color: C.dim
      }
    ...
    }, seancesAn.length), " s\xE9ances \xB7", " ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: C.text,
        ...NUMS
      }
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, sMoisPrec.length > 0 && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, "Toutes s\xE9ances confondues \u2014 la tendance de fond compte plus qu'une semaine isol\xE9e."), vol.length < 2 ? /*#__PURE__*/React.createElement("div", {
      className: "text-sm text-center py-8",
      style: {
        color: C.dim
      }
    }, "Pas encore assez de semaines pour une courbe \u2014 reviens dans quelques s\xE9ances.") : /*#__PURE__*/React.createElement(MiniLine, {
      data: vol,
      dataKey: "kg",
      color: C.yellow,
      height: 220
      ...
      return /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
        className: "text-sm font-bold mb-1"
      }, "Effort ressenti (RPE), par semaine"), /*#__PURE__*/React.createElement("div", {
        className: "text-xs mb-3",
        style: {
          color: C.dim
        }
      }, "Un RPE qui grimpe alors que les charges stagnent = fatigue accumul\xE9e.", " ", "Cette semaine : ", /*#__PURE__*/React.createElement("b", {
        style: {
          color: dernier >= 9 ? C.red : dernier >= 8 ? C.yellowDim : C.green
        }
      }, dernier, "/10")), /*#__PURE__*/React.createElement(MiniLine, {
        data: rpes,
        dataKey: "rpe",
        color: C.yellowDim,
        height: 180
      ...
      return /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
        className: "text-sm font-bold mb-2"
      }, "Sommeil et performance"), /*#__PURE__*/React.createElement("div", {
      ...
      }, cs.bien.toLocaleString("fr-FR"), " kg"), /*#__PURE__*/React.createElement("div", {
        className: "text-xs",
        style: {
          color: C.dim
        }
      }, "apr\xE8s 7 h+ (", cs.nBien, " s\xE9ances)")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        className: "text-lg",
        style: {
          ...DISPLAY,
          ...NUMS,
      ...
      }, cs.mal.toLocaleString("fr-FR"), " kg"), /*#__PURE__*/React.createElement("div", {
        className: "text-xs",
        style: {
          color: C.dim
        }
      }, "apr\xE8s moins de 7 h (", cs.nMal, ")"))), /*#__PURE__*/React.createElement("div", {
        className: "text-xs",
        style: {
          color: ecart > 0 ? C.green : C.dim
        }
      }, ecart > 0 ? `Tu soulèves ${pct} % de plus quand tu as bien dormi. Le sommeil fait partie de l'entraînement.` : "Pas d'écart net pour l'instant — continue à noter ton sommeil."));
    })());
  })(), progMode === "retro" && (() => {
    const an = new Date().getFullYear();
    const sAn = data.seances.filter(s => s.date.startsWith(String(an)));
    if (!sAn.length) return /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm text-center py-8",
      style: {
        color: C.dim
      }
    }, "Ta r\xE9tro ", an, " s'\xE9crira s\xE9ance apr\xE8s s\xE9ance. Va charger. \uD83D\uDCAA"));
    ...
    // exo fétiche
    const cnt = {};
    sAn.forEach(s => s.exos.forEach(e => {
      const k = e.nom;
      cnt[k] = (cnt[k] || 0) + 1;
    }));
    ...
    const ELEPHANT = 5000,
      VOITURE = 1400;
    const equiv = tonnage >= ELEPHANT ? `${(tonnage / ELEPHANT).toFixed(1)} éléphants 🐘` : `${(tonnage / VOITURE).toFixed(1)} voitures 🚗`;
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, {
    ...
    }, "Ta r\xE9tro"), /*#__PURE__*/React.createElement("div", {
    ...
    }, "Tu as soulev\xE9"), /*#__PURE__*/React.createElement("div", {
      className: "leading-none mt-1",
      style: {
        ...DISPLAY,
        ...NUMS,
    ...
    }, "soit ", equiv)), /*#__PURE__*/React.createElement("div", {
    ...
    }, "Ton exercice f\xE9tiche"), /*#__PURE__*/React.createElement("div", {
      className: "text-base font-bold truncate"
    }, fetiche), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.dim
      }
    }, cnt[fetiche], " fois cette ann\xE9e")))), best && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "flex items-center gap-3"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-2xl"
    }, "\uD83D\uDD25"), /*#__PURE__*/React.createElement("div", {
    ...
    }, "Ta plus grosse s\xE9ance"), /*#__PURE__*/React.createElement("div", {
      className: "text-base font-bold truncate"
    }, best.nom, " \xB7 ", fmtDate(best.date)), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        ...NUMS,
        color: C.yellowDim
      ...
      style: {
        background: shareId === "retro" ? C.card2 : C.yellow,
        color: shareId === "retro" ? C.green : "#111",
        border: shareId === "retro" ? `1px solid ${C.green}` : "none"
      }
    }, /*#__PURE__*/React.createElement(Share2, {
      size: 16
    ...
    const maxG = Math.max(...vals),
      minG = Math.min(...vals);
    const fort = groupes[vals.indexOf(maxG)],
      faible = groupes[vals.indexOf(minG)];
    const total = vals.reduce((a, b) => a + b, 0);
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-1"
    }, "\xC9quilibre du physique"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mb-2",
      style: {
        color: C.dim
      }
    }, "S\xE9ries par groupe \xB7 30 derniers jours"), total === 0 ? /*#__PURE__*/React.createElement("div", {
      className: "text-sm text-center py-8",
      style: {
        color: C.dim
      }
    }, "Pas encore de donn\xE9es \u2014 encha\xEEne quelques s\xE9ances.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(RadarMuscles, {
      volumes: v30,
      groupes: groupes
    }), /*#__PURE__*/React.createElement("div", {
    ...
    }, minG === 0 ? /*#__PURE__*/React.createElement(React.Fragment, null, "\u26A0\uFE0F ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: C.red
      }
    }, faible), " : rien en 30 jours. C'est le trou dans ta raquette.") : /*#__PURE__*/React.createElement(React.Fragment, null, "Point fort : ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: C.yellowDim
      }
    }, fort), " (", maxG, " s\xE9ries) \xB7 \xC0 rattraper : ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: maxG > minG * 2 ? C.red : C.text
      }
    }, faible), " (", minG, " s\xE9ries).")))), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-1"
    }, "Assiduit\xE9 \xB7 6 derniers mois"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mb-3",
      style: {
        color: C.dim
      }
    }, "Une case par jour \u2014 plus c'est jaune, plus tu as soulev\xE9"), /*#__PURE__*/React.createElement(Heatmap, {
    ...
    }, ng.niveau)), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-base font-bold"
    }, ng.titre), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mb-1.5",
      style: {
        ...NUMS,
        color: C.dim
      }
    }, ng.xp.toLocaleString("fr-FR"), " XP \xB7 ", (ng.xpSuivant - ng.xp).toLocaleString("fr-FR"), " avant le niveau ", ng.niveau + 1), /*#__PURE__*/React.createElement("div", {
    ...
    }, "XP gagn\xE9e : 100 par s\xE9ance \xB7 5 par s\xE9rie \xB7 250 par record \xB7 1 par 100 kg soulev\xE9s.")), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-bold mb-3"
  ...
  }, "\uD83D\uDCCB \xC9tat copi\xE9, s\xE9ance comprise \u2014 colle-le dans Importer si l'app se recharge"), finJsonCopie && /*#__PURE__*/React.createElement("div", {
    className: "fixed left-1/2 z-40 px-4 py-2 rounded-full text-xs font-bold text-center",
    style: {
      bottom: "8rem",
      transform: "translateX(-50%)",
  ...
  }, "\uD83D\uDCCB JSON de la s\xE9ance copi\xE9 \u2014 colle-le \xE0 Claude pour la sauvegarde"), savedFlash && /*#__PURE__*/React.createElement("div", {
  ...
  }, "\u2713 Enregistr\xE9"), photoView && /*#__PURE__*/React.createElement("div", {
    ...
    const hist = [];
    data.seances.forEach(s => s.exos.forEach(e => {
      if (e.nom.toLowerCase() === nom.toLowerCase()) hist.push({
        date: s.date,
        e
      });
    }));
    ...
    }, cardio ? "Dernière séance" : "Record"), /*#__PURE__*/React.createElement("div", {
      className: "text-2xl",
      style: {
        ...DISPLAY,
        ...NUMS,
    ...
    }, cardio ? `${hist[hist.length - 1].e.dureeCardio} min` : rec ? `${fmtKg(rec.poids)} kg${rec.parBras ? "/bras" : ""}` : "—")), /*#__PURE__*/React.createElement("div", {
    ...
    }, hist.length, " fois"), niv && /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-black",
      style: {
        color: niv.color
      }
    }, niv.label))), hist.length > 0 && joursDepuis(hist[hist.length - 1].date) > 60 && /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-2",
      style: {
        color: C.yellowDim
      }
    }, "\u23F3 Pas refait depuis ", joursDepuis(hist[hist.length - 1].date), " jours \u2014 \xE7a se perd vite, pense \xE0 le remettre au programme."), sg && !cardio && /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-2",
      style: {
        ...NUMS,
    ...
    }, "Note perso"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs",
      style: {
        color: C.yellowDim
      }
    }, (data.notesExo || {})[nom.toLowerCase()] ? "Modifier" : "Ajouter")), /*#__PURE__*/React.createElement("div", {
      className: "text-sm",
      style: {
        color: (data.notesExo || {})[nom.toLowerCase()] ? C.text : C.dim
      }
    }, (data.notesExo || {})[nom.toLowerCase()] || "Ex: réglage du siège, technique à retenir…")), rp && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-xs font-bold mb-2"
    ...
    }, /*#__PURE__*/React.createElement("span", null, "d\xE9b ", fmtKg(rp.paliers[0])), /*#__PURE__*/React.createElement("span", null, "nov ", fmtKg(rp.paliers[1])), /*#__PURE__*/React.createElement("span", null, "int ", fmtKg(rp.paliers[2])), /*#__PURE__*/React.createElement("span", null, "av ", fmtKg(rp.paliers[3])))), !cardio && /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement(SchemaMouvement, {
      nom: nom
    }), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1 mb-2 text-center",
      style: {
    ...
    }, "trait plein = d\xE9part \xB7 pointill\xE9s = arriv\xE9e"), (TECHNIQUE[mouvementOf(nom)] || TECHNIQUE.generique).map((t, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      className: "text-xs leading-relaxed",
      style: {
        color: C.text
    ...
    }, "\xB7"), " ", t))), /*#__PURE__*/React.createElement(Card, null, /*#__PURE__*/React.createElement("div", {
      className: "text-xs font-bold mb-1"
    }, "Historique"), hist.length === 0 ? /*#__PURE__*/React.createElement("div", {
      className: "text-sm py-3 text-center",
      style: {
        color: C.dim
      }
    }, "Jamais fait pour l'instant.") : [...hist].reverse().map(({
      date,
      e
    ...
    }, fmtShort(date)), /*#__PURE__*/React.createElement("div", {
      className: "text-sm flex-1",
      style: NUMS
    }, /*#__PURE__*/React.createElement(Dot, {
      ressenti: e.ressenti
    }), e.type === "cardio" ? `${e.dureeCardio} min${e.distance ? ` · ${fmtKg(e.distance)} km` : ""}` : `${e.series}×${e.reps} @ ${fmtKg(e.poids)} kg${e.parBras ? "/bras" : ""}`), e.pr && /*#__PURE__*/React.createElement(PrBadge, null)))), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "S\xE9rie ", /*#__PURE__*/React.createElement("b", {
      style: {
        color: C.text
      }
    }, Math.min(ec.sets.length + 1, ec.prevu || ec.sets.length + 1), "/", ec.prevu || "?"), ec.parBras ? " · par bras" : "", " \xB7 repos ", fmtRepos(ec.reposSec))), /*#__PURE__*/React.createElement("button", {
    ...
    }, "R\xE9duire")), ec.sets.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "flex gap-2 overflow-x-auto pb-1 mb-1",
      style: {
        scrollbarWidth: "none"
      }
    }, ec.sets.map((s, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
    ...
    }, i + 1, "\xB7 ", fmtKg(s.poids), "\xD7", s.reps))), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 flex flex-col items-center justify-center"
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative flex items-center justify-center" + (enRepos ? "" : " pl-go"),
      style: {
        width: 220,
        height: 220
      }
      ...
      r: R,
      fill: "none",
      stroke: enRepos ? C.yellow : C.green,
      strokeWidth: "9",
      strokeLinecap: "round",
      strokeDasharray: circ,
    ...
    }, enRepos ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      style: {
        ...DISPLAY,
        ...NUMS,
        fontSize: 60,
    ...
    }, Math.floor(left / 60), ":", String(left % 60).padStart(2, "0")), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1 uppercase tracking-widest",
      style: {
        color: C.dim
      }
    }, "repos")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      style: {
        ...DISPLAY,
        fontSize: 46,
        lineHeight: 1,
    ...
    }, "\xC0 TOI"), /*#__PURE__*/React.createElement("div", {
      className: "text-sm mt-1",
      style: {
        color: C.dim
      }
    }, "s\xE9rie ", ec.sets.length + 1, ec.prevu ? ` / ${ec.prevu}` : "")))), enRepos && /*#__PURE__*/React.createElement("div", {
      className: "flex gap-3 mt-5 w-full"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => setRest({
        ...rest,
        endsAt: Math.max(Date.now(), rest.endsAt - 30000),
        total: Math.max(30, rest.total - 30)
      }),
    ...
    }, "\u221230 s"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setRest({
        ...rest,
        endsAt: rest.endsAt + 30000,
        total: rest.total + 30
      }),
      className: "flex-1 rounded-2xl py-5 text-lg font-black",
      style: {
      ...
      return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
        ...
        style: {
          background: derniere ? C.green : C.yellow,
          color: "#111"
        }
      }, /*#__PURE__*/React.createElement(Check, {
        size: 22
      }), " ", derniere ? `Dernière série (${n}/${ec.prevu}) → fin` : `Série ${n}${ec.prevu ? "/" + ec.prevu : ""} faite`), /*#__PURE__*/React.createElement("div", {
        className: "flex gap-2 mt-2"
      }, /*#__PURE__*/React.createElement("button", {
      ...
      }, "+ 1 s\xE9rie"), !derniere && /*#__PURE__*/React.createElement("button", {
        onClick: finirSerieEtExo,
        className: "pl-tap flex-1 rounded-2xl py-3.5 font-bold",
        style: {
          background: C.text,
      ...
      }, "Stop apr\xE8s celle-ci"), /*#__PURE__*/React.createElement("button", {
        onClick: finirExo,
      ...
      }, "Finir sans", /*#__PURE__*/React.createElement("br", null), "ajouter")));
    })()));
  })(), rest && !(ec && focusOuvert) && (() => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        color: done ? C.green : C.yellowDim
      }
    }, done ? "Go — à toi !" : "Repos"), ec ? /*#__PURE__*/React.createElement("div", {
    ...
    }, done ? "GO" : `${Math.floor(left / 60)}:${String(left % 60).padStart(2, "0")}`), !done && /*#__PURE__*/React.createElement("div", {
      className: "text-sm mt-2",
      style: {
        color: C.dim,
        ...NUMS
      }
    }, "sur ", fmtRepos(rest.total))))), done ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
      onClick: fermer,
    ...
    }, "Encore ", fmtRepos(rest.total), " de repos")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "flex gap-3 mb-3"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => setRest({
        ...rest,
    ...
    }, "Passer le repos"), /*#__PURE__*/React.createElement("button", {
      onClick: () => relancer(rest.total),
      className: "w-full rounded-2xl py-4 text-base font-bold mt-3",
      style: {
        background: C.card2,
  ...
  }, "Ton carnet de muscu \u2014 pens\xE9 pour la salle, pour de vrai."), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col gap-3.5 mt-8"
  }, [["🏋️", "Chaque séance loggée", "Charges, séries, repos, ressenti — en 2 taps."], ["📈", "Records & progression", "1RM, plateaux détectés, tonnage, courbes."], ["📷", "Retrouve ta machine", "Photo perso + vidéo technique pour chaque exo."]].map(([e, t, s], i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "flex items-start gap-3"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Ton objectif par semaine ?"), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2"
  }, [2, 3, 4, 5].map(n => /*#__PURE__*/React.createElement(Chip, {
    key: n,
    active: onbGoal === n,
    onClick: () => setOnbGoal(n)
  }, n, " s\xE9ances")))), /*#__PURE__*/React.createElement("div", {
    className: "flex-1",
    style: {
      minHeight: 24
    }
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, prs.length > 0 && /*#__PURE__*/React.createElement(Confetti, null), /*#__PURE__*/React.createElement("div", {
      className: "pl-atoi",
      style: {
        fontSize: 52,
        lineHeight: 1
    ...
    }, s.nom.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    ...
    }, k)))), prs.length > 0 && /*#__PURE__*/React.createElement("div", {
      className: "mt-4 w-full rounded-2xl p-3",
      style: {
        maxWidth: 340,
        background: "linear-gradient(180deg, rgba(245,197,24,.15), rgba(245,197,24,.04))",
      ...
      size: 13
    }), " ", prs.length, " nouveau", prs.length > 1 ? "x" : "", " record", prs.length > 1 ? "s" : "", " !"), /*#__PURE__*/React.createElement("div", {
      className: "text-xs mt-1 leading-relaxed",
      style: {
        ...NUMS,
        color: C.text
    ...
    }, /*#__PURE__*/React.createElement(ShowerHead, {
      size: 13
    }), " Pense \xE0 la douche"));
  })(), /*#__PURE__*/React.createElement("div", {
    className: "pl-tabbar"
  }, /*#__PURE__*/React.createElement("div", {
    className: "pl-tabbar-in"
  }, [{
    id: "accueil",
    label: "Accueil",
    Icon: Home
    ...
    key: id,
    onClick: () => setTab(id),
    className: "pl-tab pl-tap" + (tab === id ? " on" : "")
